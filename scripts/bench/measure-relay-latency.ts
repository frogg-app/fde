...
const OFFER = {
  ...
  relay: { endpoint: process.env.FROGG_RELAY_ENDPOINT ?? "" },
};
...
async function main() {
  ...
  await measurePings("Configured relay", relayClient, PING_COUNT, WARMUP_COUNT);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const ws = new WebSocket(
      `wss://${process.env.FROGG_RELAY_ENDPOINT}/ws?serverId=latency_probe_${Date.now()}&role=client&clientId=probe_${i}`,
    );
    await new Promise<void>((resolve, reject) => {
      ws.on("open", () => {
