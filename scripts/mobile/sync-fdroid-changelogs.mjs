...
// CommonJS on purpose: apps/ui/app.config.js and the Expo config plugin both
// require it, so the ABI table has exactly one definition across all three callers.
import { getFdroidVersionCodes as defaultGetFdroidVersionCodes } from "../apps/ui/native-release-version.js";
import { parseChangelogBody, parseChangelogEntries } from "../release/changelog-utils.mjs";
...
function usageAndExit(code = 1) {
  const usage = `
Usage: node scripts/mobile/sync-fdroid-changelogs.mjs [options]
