...
STATE_DIR="${FROGG_MOBILE_E2E_STATE_DIR:-${REPO_ROOT}/.dev/agent-device-e2e}"
ARTIFACTS_DIR="${FROGG_MOBILE_E2E_ARTIFACTS_DIR:-${REPO_ROOT}/.dev/agent-device-artifacts}"
METRO_PORT="${FROGG_MOBILE_E2E_METRO_PORT:-8081}"
METRO_PID=0
...
METRO_RESULT="$({
  AGENT_DEVICE_STATE_DIR="${STATE_DIR}" agent-device metro prepare \
    --project-root "${REPO_ROOT}/apps/ui" \
    --kind expo \
    --port "${METRO_PORT}" \
...
AGENT_DEVICE_STATE_DIR="${STATE_DIR}" agent-device test \
  "${REPO_ROOT}/apps/ui/e2e/mobile/agent-device" \
