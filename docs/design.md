...
Tokens — every color, font size, weight, spacing step, radius, icon size — live in `apps/ui/src/styles/theme.ts`.

---

## 1. Character

FDE is minimal, spacious, quiet, confident. Whitespace is deliberate. Nothing crowds, nothing decorates, nothing apologizes. A row, a label, a control. That is the bar.
...
Primitives live in `apps/ui/src/components/ui/` and `apps/ui/src/components/headers/`. Card and row layout live in `apps/ui/src/styles/settings.ts`. Section structure lives in `apps/ui/src/screens/settings/settings-section.tsx`.

A pressable styled to look like a button is wrong; the button is `<Button>` (`apps/ui/src/components/ui/button.tsx`). A bare `<Text>` styled to look like a section header is wrong; the section header is `<SettingsSection>` (`apps/ui/src/screens/settings/settings-section.tsx`). A custom `Modal` for a confirmation is wrong; the confirmation is `confirmDialog` (`apps/ui/src/utils/confirm-dialog.ts`). A hand-rolled overflow menu is wrong; the menu is `<DropdownMenu>` (`apps/ui/src/components/ui/dropdown-menu.tsx`). A hand-rolled status pill is wrong; the pill is `<StatusBadge>` (`apps/ui/src/components/ui/status-badge.tsx`).
...
- **Screen titles** — the title in app chrome — use `<ScreenTitle>` (`apps/ui/src/components/headers/screen-title.tsx`), which renders `fontSize.base` at weight `400` on compact and `300` on desktop. The New workspace hero is the only larger product title; it uses `fontSize["2xl"]` (`apps/ui/src/screens/new-workspace-screen.tsx`).
- **Structural labels** use `fontWeight.medium`. This applies to section labels above a stack of rows (`apps/ui/src/components/agent-list.tsx:519-523`, `apps/ui/src/components/keyboard-shortcuts-dialog.tsx:63-67`), form field labels above an input inside a modal (`apps/ui/src/components/add-host-modal.tsx:19-23`, `apps/ui/src/components/pair-link-modal.tsx:24-28`), the title at the top of a modal/sheet/dialog (`apps/ui/src/components/adaptive-modal-sheet.tsx:90-94`, `apps/ui/src/components/ui/combobox.tsx:1607-1611`, `apps/ui/src/components/welcome-screen.tsx:48-53`), action button labels in tight components such as the sidebar callout actions (`apps/ui/src/components/sidebar-callout.tsx:218-221`), and inline data emphasis on dense metadata rows (`apps/ui/src/components/git-diff-pane.tsx:2322-2327`, `apps/ui/src/components/file-explorer-pane.tsx:1115-1122`).
- **Content** uses `fontWeight.normal`. This applies to settings rows (`apps/ui/src/styles/settings.ts`), sidebar primary list-item titles (`apps/ui/src/components/sidebar-workspace-list.tsx:2680-2686`, `apps/ui/src/components/agent-list.tsx:572-578`), `<Button>` text (`apps/ui/src/components/ui/button.tsx:80-84`), `<StatusBadge>` text (`apps/ui/src/components/ui/status-badge.tsx:56-60`), and `<SidebarCallout>` titles (`apps/ui/src/components/sidebar-callout.tsx:175-180`).
...
Destructive is a color, not a click. Restart-daemon and remove-host are `<Button variant="outline">` in the row trailing slot; the destructive surface only appears inside the `confirmDialog` (`apps/ui/src/screens/settings/host-page.tsx:541-547`). Workspace archive opens a confirm dialog before any red appears (`apps/ui/src/components/sidebar-workspace-list.tsx`). Red appears after the user has indicated intent.

---

## 4. Buttons

The button is `<Button>` (`apps/ui/src/components/ui/button.tsx`). It has five variants. Each has one job.
...
`outline` is the low-frequency action that lives on a row — transparent with `borderAccent`. Restart, Remove, Update on host detail (`apps/ui/src/screens/settings/host-page.tsx:585-594`).

`ghost` is structural and non-committal — no border, no fill. Back arrows, header toggles, "Load more" footers (`apps/ui/src/screens/sessions-screen.tsx:54-63`), more-affordances. Ghost is used when the affordance is part of the chrome, not a decision.
...
A logical block of related rows lives inside a card — one border around the whole group. The card primitive is `settingsStyles.card`; the keyboard-shortcuts dialog uses the same shape inline (`apps/ui/src/components/keyboard-shortcuts-dialog.tsx:68-73`). The border defines what belongs together.

Rows after the first inside a card carry `settingsStyles.rowBorder` — a single top border. The first row never has one. The same divider pattern appears in the keyboard-shortcuts dialog rows (`apps/ui/src/components/keyboard-shortcuts-dialog.tsx:74-83`). Rows do not need their own background to feel separated.

A list that is itself the page content — sidebar items in `sidebar-workspace-list.tsx`, the workspace list, the agent list (`apps/ui/src/components/agent-list.tsx`) — uses spacing and surface, not borders, to separate items. Rows-in-a-card is an interior pattern; lists-as-pages are not.

Pane chrome — the workspace pane header, the file-explorer header, the diff pane header — uses a single bottom border to separate the header from the content (`apps/ui/src/components/git-diff-pane.tsx:2328-2331`). One border, no shadow.
...
`<AdaptiveModalSheet>` is for a focused task. Multi-field forms (`apps/ui/src/components/add-host-modal.tsx`, `apps/ui/src/components/pair-link-modal.tsx`, `apps/ui/src/components/project-picker-modal.tsx`), confirmations with detail, anything that earns a backdrop. Bottom sheet on compact, centered card on desktop. Raw `Modal` is wrong for any of these.
...
Settings detail pages, the projects detail page, and any list+detail content sit inside a centered, max-width 720 column (`apps/ui/src/screens/settings-screen.tsx`, `apps/ui/src/screens/projects-screen.tsx`). Lines stay readable, the eye does not have to track wide horizontal distances. Form modals carry their own narrower content frame (`apps/ui/src/components/add-host-modal.tsx`).

Workspace and chat surfaces use the full width — these are working surfaces, not reading surfaces. The composer carries `MAX_CONTENT_WIDTH` from `apps/ui/src/constants/layout.ts` to keep lines readable while letting the workspace pane fill the rest.

Sections sit apart. `<SettingsSection>` owns its own bottom margin; the next thing is wrapped in another `<SettingsSection>`. The agent-list `sectionHeading` carries the same `marginTop`/`marginBottom` rhythm (`apps/ui/src/components/agent-list.tsx:511-517`). Adding `marginBottom` to a section is wrong.

A section or group explains itself through the `info` prop on `<SettingsSection>` or `<SettingsGroup>` — an info icon beside the header that opens a tooltip (`apps/ui/src/screens/settings/settings-info-tip.tsx`). A muted paragraph between the header and the card is wrong: it sits in the section's own gap, so it reads as a second heading rather than as prose belonging to the header. Explanatory copy that describes one row belongs to that row, as `settingsStyles.rowHint` inside the card.
...
The list+detail pattern is canonical and reused across surfaces. The settings shell (`apps/ui/src/screens/settings-screen.tsx`) and the projects screen (`apps/ui/src/screens/projects-screen.tsx`) implement it identically:
...
Sentence case. "Pair a device", "Danger zone", "Restart daemon", "Inject FDE tools", "No sessions yet", "Load more". Proper nouns retain casing — FDE, Beta, Stable, Local. Title case is wrong.

No trailing periods on row titles, labels, or buttons. No trailing period on a single-clause hint: "What happens when you press Enter while the agent is running" (`apps/ui/src/screens/settings-screen.tsx:271-272`). Periods exist inside multi-sentence prose: "Restarts the daemon process. The app will reconnect automatically."

Empty-state strings are short noun phrases or short sentences: "No projects yet", "Select a project", "No sessions yet" (`apps/ui/src/screens/sessions-screen.tsx:74-76`), "Host not found".
...
Error copy is direct. "Unable to remove host" (`apps/ui/src/screens/settings/host-page.tsx:697`), not "Sorry, we couldn't remove the host." Recovery instructions are concrete: "Wait for it to come online before restarting." Errors describe state; they do not editorialize.
...
Loading is inline by default. `<LoadingSpinner size={14} color={foregroundMuted} />` sits next to the thing it relates to (`apps/ui/src/screens/settings/providers-section.tsx:227-231`). Page-level loading is a centered `<LoadingSpinner size="large">` (`apps/ui/src/screens/sessions-screen.tsx:69-72`). Card-level loading is a single short line, not a spinner. In-row dropdown items use `<DropdownMenuItem status="pending" pendingLabel="Removing...">`; the menu item handles its own pending state.

Empty states are short noun phrases. Centered, muted, one or two lines. Sessions screen pairs the empty noun with a single ghost button to navigate back (`apps/ui/src/screens/sessions-screen.tsx:74-81`); that pairing is the maximum elaboration. Illustrations and CTAs disguised as empty states are wrong.

Inline errors are a single sentence in `palette.red[300]` `xs`, sitting under the field or inside the card it relates to (`apps/ui/src/screens/settings/providers-section.tsx:115-119`).

Page-level alerts — informational notices, success confirmations, warnings, or recoverable errors that need a small visible block on the page — use `<Alert>` (`apps/ui/src/components/ui/alert.tsx`). Variants: `default`, `info`, `success`, `warning`, `error`. The chrome is quiet by design: a 1px tinted border, transparent background, a small variant-tinted icon, the title in the variant accent, the description in `foregroundMuted`. Actions go in the `children` slot as `<Button variant="outline" size="sm">` — recovery actions are low-frequency and outline keeps them quiet alongside the alert's accent (`apps/ui/src/screens/project-settings-screen.tsx`). One `<Alert>` at a time per region.

Sidebar callouts — cross-cutting alerts that apply across the whole app, like worktree setup, Rosetta install, and desktop update available — register through `useSidebarCallouts()` and render in the left sidebar via `<SidebarCallout>` (`apps/ui/src/components/sidebar-callout.tsx`). The chrome (top-border-only, full-width action buttons) is tuned for that ~280px column. Canonical sources: `apps/ui/src/components/worktree-setup-callout-source.tsx`, `apps/ui/src/desktop/updates/rosetta-callout-source.tsx`, `apps/ui/src/desktop/updates/update-callout-source.tsx`. Never import `<SidebarCallout>` into a page — that's what `<Alert>` is for.
...
Partial failure (a list mostly fine but one source errored) is a bordered banner above the list, listing each failure in red-300 `xs` (`apps/ui/src/screens/projects-screen.tsx:151-159`). The list still renders.
...
The row anatomy is a content column with an optional trailing slot. Inside a card the row is `settingsStyles.row`. Inside a sidebar list the row carries its own padding and `borderRadius.lg` per item (`apps/ui/src/components/sidebar-workspace-list.tsx:2614-2625`).

Rows that drill into a detail lead with a chevron in the trailing slot (`ChevronRight`, `iconSize.sm`, `foregroundMuted`). The whole row is the `<Pressable>`. Pair-device row (`apps/ui/src/screens/settings/host-page.tsx:644-668`), provider row (`apps/ui/src/screens/settings/providers-section.tsx:92-132`), project row in the projects list. Chevron means navigation.

Kebab menus (`<DropdownMenu>` with `<MoreVertical size={14} />` trigger) are for actions on the row, not navigation. Trigger style: `padding: 2`, `borderRadius: 4`, hover background `surface2`. Menu position: `align="end"`. Items use `<DropdownMenuItem leading={<Icon size={14} color={foregroundMuted} />} ...>`. Visibility is `isHovered || isTouchPlatform` — hover-revealed on web, always visible on native (`apps/ui/src/components/sidebar-workspace-list.tsx:684-770`).
...
| Pattern                                             | Reference                                                                                                                                                                                                                                                                       |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| List+detail (compact stack, desktop sidebar+pane)   | `apps/ui/src/screens/settings-screen.tsx`, `apps/ui/src/screens/projects-screen.tsx`                                                                                                                                                                                            |
| Detail card+row                                     | `apps/ui/src/screens/settings/host-page.tsx`, `apps/ui/src/screens/settings/providers-section.tsx`                                                                                                                                                                              |
| Section grouping inside a card list                 | `apps/ui/src/screens/settings/settings-section.tsx`                                                                                                                                                                                                                             |
| Form modal (label + input fields, primary + cancel) | `apps/ui/src/components/add-host-modal.tsx`, `apps/ui/src/components/pair-link-modal.tsx`, `apps/ui/src/components/project-picker-modal.tsx`                                                                                                                                    |
| Destructive confirmation                            | `confirmDialog` invoked from `apps/ui/src/screens/settings/host-page.tsx:541-547`                                                                                                                                                                                               |
| Centered hero / first-run                           | `apps/ui/src/components/welcome-screen.tsx`                                                                                                                                                                                                                                     |
| Sidebar list (workspaces, hosts)                    | `apps/ui/src/components/sidebar-workspace-list.tsx`, `apps/ui/src/components/left-sidebar.tsx`                                                                                                                                                                                  |
| Live list of items with sections (agents)           | `apps/ui/src/components/agent-list.tsx`                                                                                                                                                                                                                                         |
| Historical list (sessions)                          | `apps/ui/src/screens/sessions-screen.tsx`                                                                                                                                                                                                                                       |
| Workspace pane (multi-tab, split)                   | `apps/ui/src/screens/workspace/workspace-screen.tsx`                                                                                                                                                                                                                            |
| Composer / message input                            | `apps/ui/src/components/composer.tsx`, `apps/ui/src/components/message-input.tsx`                                                                                                                                                                                               |
| Pane chrome with single bottom border               | `apps/ui/src/components/git-diff-pane.tsx`, `apps/ui/src/components/file-explorer-pane.tsx`, `apps/ui/src/components/terminal-pane.tsx`                                                                                                                                         |
| Page-level alert (info / success / warning / error) | `apps/ui/src/components/ui/alert.tsx`, `apps/ui/src/screens/project-settings-screen.tsx`                                                                                                                                                                                        |
| Sidebar callout (cross-cutting alert)               | `apps/ui/src/components/sidebar-callout.tsx`, `apps/ui/src/contexts/sidebar-callout-context.tsx`, `apps/ui/src/components/worktree-setup-callout-source.tsx`, `apps/ui/src/desktop/updates/rosetta-callout-source.tsx`, `apps/ui/src/desktop/updates/update-callout-source.tsx` |
| Searchable picker                                   | `apps/ui/src/components/ui/combobox.tsx`, `apps/ui/src/components/branch-switcher.tsx`                                                                                                                                                                                          |
