...
The file explorer uses colored SVG icons from [`material-icon-theme`](https://github.com/material-extensions/vscode-material-icon-theme) (installed as a dev dependency in `apps/ui`).

## How it works

Two files:

- `apps/ui/src/components/material-file-icons.ts` — the vendor table. `SVG_ICONS` maps icon
...
- `apps/ui/src/components/file-icon-svg.ts` — `getFileIconSvg(fileName)`, the only thing
...
in `apps/ui/src/utils/color.ts`, OKLab — scaling HSL saturation instead would darken the
