...
`apps/ui/src/workspace-tabs/explorer-sidebar.ts` owns show, hide, toggle, and view selection.
...
`apps/ui/src/workspace-tabs/open-supporting-view.ts` owns semantic Changes and pull-request
...
`apps/ui/src/workspace-tabs/open-beside.ts` owns content opened beside the user's work. The
