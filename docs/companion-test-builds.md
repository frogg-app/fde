...
baseline. They are not a signed production release.

## Artifacts

In `dist/companion-test-builds/`:
...
The [Companion prerelease](https://github.com/frogg-app/fde/releases/tag/companion-preview-20260912-1)
contains the APK, daemon, checksums and a standalone installer. On Linux x64:

```sh
curl -fL https://github.com/frogg-app/fde/releases/download/companion-preview-20260912-1/install-companion-preview.sh -o /tmp/install-companion-preview.sh && bash /tmp/install-companion-preview.sh
```

The installer verifies the pinned archive checksum, starts the daemon with relay
enabled on port 6798 and prints pairing instructions. It uses separate
`~/.local/share/fde-companion-preview-20260912-1` and
`~/.fde-companion-preview-20260912-1` directories. It does not replace the regular
`fde` command or install a system service. Set `FDE_COMPANION_LISTEN` to choose
another port. The provider CLI must already be installed and signed in on this host.
...
./fde-daemon-0.2.14-linux-x64/bin/fde start \
  --home "$FDE_COMPANION_TEST_HOME" --listen 0.0.0.0:6798 --no-relay --web-ui
./fde-daemon-0.2.14-linux-x64/bin/fde status --home "$FDE_COMPANION_TEST_HOME"
```

Use an unused port if 6798 is occupied. Keep the printed state path for subsequent
commands. Open `http://HOST_LAN_IP:6798` to confirm the browser UI loads. For a
...
choose **Start Companion**. Allow microphone access. Keep **Codex voice (preview)**
...
ANDROID_HOME=/path/to/android-sdk node scripts/release/build-android-apk.mjs \
  --app-variant development --low-memory --out-dir dist/companion-test-builds
npm run build:server
npm run build:daemon-web-ui
node scripts/release/build-daemon-bundle.mjs \
  --target linux-x64 --out-dir dist/companion-test-builds
```
