...
do not use `createFdeDaemon` as a product launch path.
...
const fdeHomeRoot = await mkdtemp(path.join(os.tmpdir(), "fde-test-"));
...
const daemon = await createFdeDaemon(
  {
    ...
    relayEnabled: false,
    relayEndpoint: "<configured-relay-host>:443",
...
For simpler cases, `createTestFdeDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. In test harnesses, use `createFdeDaemon` directly when you need it (see quick start above).
