...
The image source lives in [`deploy/docker/`](../deploy/docker/).

## How it works

The official image:

- builds `@fde/server` and `@fde/cli` from source-built workspace tarballs
...
```bash
docker build -f deploy/docker/base/Dockerfile -t paseo:local .
```
...
docker build \
  ...
  -t paseo:0.1.102 \
  -f deploy/docker/base/Dockerfile \
  .
```
