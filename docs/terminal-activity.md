...
FDE surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
OpenCode uses a server plugin instead of command hooks. The plugin listens to OpenCode bus events and emits these FDE hook events:
...
removes FDE's marker-matched hooks on disable. `fde hooks` keeps working regardless — the gate
...
When enabled, FDE installs provider hooks globally:

- Claude hooks are written to `~/.claude/settings.json` (or `CLAUDE_CONFIG_DIR/settings.json` when that override is set).
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each FDE hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/fde-terminal-activity.js` (or `~/.config/opencode/plugins/fde-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).

Installation is marker-based/idempotent for config hooks and exact-file/idempotent for the OpenCode plugin. FDE preserves user hooks, removes only its own marker-matched command hooks, and leaves hooks installed across daemon shutdown. Outside a FDE terminal they are inert because the command or plugin is gated on `FDE_TERMINAL_ID`.
...
```sh
[ -n "$FDE_TERMINAL_ID" ] && "${FDE_HOOK_CLI:-fde}" hooks claude <event>
```

Codex also receives the Windows equivalent:

```bat
if defined FDE_TERMINAL_ID (if defined FDE_HOOK_CLI ("%FDE_HOOK_CLI%" hooks codex <event>) else (fde hooks codex <event>))
```

The daemon resolves the current CLI through `FDE_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `FDE_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `fde` if the hook env is missing and no-ops outside FDE terminals because the `FDE_TERMINAL_ID` gate remains first. FDE also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `fde hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
