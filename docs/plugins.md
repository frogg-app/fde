...
See `examples/plugins/timeline-items` for the complete contract.
...
See `examples/plugins/local-plugin` for a native surface, `examples/plugins/linear` for a complete
attachment-source example, `examples/plugins/timeline-items` for timeline projection, and
`examples/plugins/catppuccin` for a theme.
