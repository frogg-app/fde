...
- Composer UI and submit/draft behavior live in `apps/ui/src/composer/`; screens and panels should integrate it from there instead of dropping composer internals into `components/`, `hooks/`, or `screens/workspace/`
...
| Surface                 | Key                                                      | Source                                             |
| ----------------------- | -------------------------------------------------------- | -------------------------------------------------- |
| Git status              | `checkoutStatusQueryKey(serverId, cwd)`                  | `apps/ui/src/git/query-keys.ts`                    |
| Git diff                | `checkoutDiffQueryKey(serverId, cwd, mode, baseRef, ws)` | `apps/ui/src/git/query-keys.ts`                    |
| Forge change request    | `checkoutPrStatusQueryKey(serverId, cwd)`                | `apps/ui/src/git/query-keys.ts`                    |
| Change request timeline | `prPaneTimelineQueryKey({ serverId, cwd, prNumber })`    | `apps/ui/src/git/pull-request-panel/query-keys.ts` |
| File preview content    | `["workspaceFile", serverId, cwd, path]`                 | `apps/ui/src/components/file-pane.tsx`             |
| File explorer listings  | fetched via `listDirectory(workspaceRoot, path)`         | `apps/ui/src/hooks/use-file-explorer-actions.ts`   |
...
| ---------------------------- | -------------------------------------------------- | -------------------------------------------------------- |
| Review draft comments        | `buildReviewDraftKey`                              | `apps/ui/src/review/store.ts`                            |
| Working diff comparison      | `workingDiffComparisonKey` (in-memory)             | `apps/ui/src/git/working-diff-comparison/state.ts`       |
| Composer attachments         | `buildWorkspaceAttachmentScopeKey`                 | `apps/ui/src/attachments/workspace-attachments-store.ts` |
| File explorer nav/open state | `fileExplorer` map keyed `workspace:{workspaceId}` | `apps/ui/src/hooks/use-file-explorer-actions.ts`         |
| File explorer expanded paths | `expandedPathsByWorkspace[workspaceStateKey]`      | `apps/ui/src/stores/panel-store/state.ts`                |
