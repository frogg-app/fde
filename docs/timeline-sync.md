...
Provider message IDs are not guaranteed for every displayed item. FDE-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
...
the FDE timeline, so one visible response can span several canonical turns without a user message
...
- App sync planning: `apps/ui/src/timeline/timeline-sync-plan.ts`
- App viewed-agent synchronization: `apps/ui/src/timeline/viewed-timeline-sync.ts`
- App stream/timeline reducer: `apps/ui/src/timeline/session-stream-reducers.ts`
- Session wiring: `apps/ui/src/contexts/session-context.tsx`
