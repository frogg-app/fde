...
One engine, in `apps/ui/src/components/ui/menu/`. `dropdown-menu.tsx` and `context-menu.tsx`
...
(`apps/ui/src/screens/workspace/workspace-header-menu.tsx`): compact has no tab strip, so it
