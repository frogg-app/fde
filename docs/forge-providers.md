# Adding a Git Forge to FDE

FDE's forge layer is a registry/manifest system. A forge is a runtime concern:
...
3. **App modules** - a forge splits into a pure logic half and a view half so
   ...
   - `apps/ui/src/git/forges/acme.ts` - logic: `id`, optional `urlGrammar`
     ...
     `apps/ui/src/git/forges/index.ts`.
   - `apps/ui/src/git/forges/acme.view.tsx` - view: `icon` (SVG component
     under `apps/ui/src/components/icons/`), optional `brandColor`, optional
     `paneContributions`. Register in `CLIENT_FORGE_VIEW_MODULES` in
     `apps/ui/src/git/forges/view.ts`.
...
allowlist. Self-hosted detection is a trust gate: FDE only talks to a forge
...
Two registries live under `apps/ui/src/git/forges/`:
...
5. Add `apps/ui/src/git/forges/acme.ts` (logic) and
   `apps/ui/src/git/forges/acme.view.tsx` (view).
