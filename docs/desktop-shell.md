...
| ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `platform`, `windowChromeMode`                                                        | constants from `tauri-plugin-os` / config. Chrome mode: `native-mac` on macOS, `custom-windows`/`custom-linux` elsewhere (decorations off, UI draws the titlebar as it does today; see Window dragging below).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `network.localAddresses()`, `network.reverseLookup(ip)`, `network.probeIdentity(url)` | optional extension point declared by the UI (`apps/ui/src/desktop/host.ts`, `DesktopNetworkBridge`) for the "Servers on your network" scan (`apps/ui/src/network-scan/`). `localAddresses()` resolves to this machine's IPv4 addresses as CIDR strings (`192.168.1.20/24`; interfaces that are up, minus loopback and link-local) from `network_local_addresses`, so the scan sweeps the right /24 subnets; `reverseLookup(ip)` resolves to the PTR name or `null` from `network_reverse_lookup` (1 s budget); `probeIdentity(url)` GETs a daemon's `/api/identity` or `/api/health` from Rust (`network_probe_identity`, 700 ms) and resolves `{status, body}` or rejects with the transport error. The scanner prefers `probeIdentity` whenever it exists because the webview's own `fetch` is subject to Chromium's local-network rules (see LAN scan below). When the member is absent the UI falls back to `fetch`, the page host and the common private subnets. |
| `menu.*`, `editor.*`, `browser.*`                                                     | not implemented in milestone 1. Absent members make the UI hide those features.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

### Window dragging (custom chrome)

Windows and Linux run without native decorations, so the UI draws the title bar. The UI marks
drag surfaces two ways: `-webkit-app-region: drag` (Electron's convention, which WebView2 and
WebKitGTK ignore) and `data-tauri-drag-region`. Tauri's own injected handler only reacts when
the element under the pointer _itself_ carries the attribute, and the header's text and icons
sit above the surface, so it never fired. The bridge therefore installs its own capture-phase
`mousedown` handler (`apps/desktop/src/drag-region.ts`, custom chrome only): it walks from the
pressed element up its ancestors, stops at the first interactive element (`button`, `a[href]`,
form controls, `[contenteditable]`, `role=button|textbox|menuitem|tab|slider|link|checkbox|switch`,
`data-tauri-drag-region="false"` or computed `app-region: no-drag`) and otherwise starts a
window drag at the first ancestor with `data-tauri-drag-region` or computed `app-region: drag`
(`core:window:allow-start-dragging`). The second press of a double-click (`detail === 2`)
toggles maximize instead, as Tauri's handler does, since the webview may not see `dblclick`
once the OS owns the pointer. Nothing happens while the window is fullscreen.

The UI marks its top-bar containers with `TITLEBAR_DRAG_SURFACE_DATASET` (React Native
`dataSet`, rendered as `data-tauri-drag-region`): `ScreenHeader` (only when it owns a window
chrome corner, so a modal's header is not a drag surface), the window-controls bar, the
workspace pane tab row and its fallback strip, the sidebar header, the explorer rails and the
landing screen's top strip. Interactive children are excluded by the handler, so buttons need
no opt-out. `apps/desktop/test/drag-region.harness.mjs` drives the exported UI in headless
Chromium against a stubbed `__TAURI_INTERNALS__` and checks which presses invoke
`plugin:window|start_dragging` (`npm run test:harness` in `apps/desktop`).

### LAN scan (`network.*`)

Two things kept the Windows shell from finding a daemon that answered `curl`:

- `localAddresses()` returns CIDR strings, but the scanner's private-range check parsed the
  whole string and rejected `192.168.1.23/24`, so every desktop scan silently fell back to the
  default subnets (the `No FDE servers found on 192.168.0.0/24, 192.168.1.0/24, 10.0.0.0/24`
  message). Fixed in `apps/ui/src/network-scan/subnets.ts`.
- WebView2 serves the app from `http://tauri.localhost` through an interception handler, so
  Chromium has no remote endpoint for the document and classifies it as a _public_ address
  space. Its Local Network Access rules then gate every `fetch` to a LAN address on a
  permission the shell never grants: the request fails with `Failed to fetch` and the daemon
  sees nothing, not even a preflight. `apps/desktop/test/network-scan.harness.mjs` reproduces
  this with a Playwright-fulfilled `tauri.localhost` document (Chromium logs "Permission was
  denied for this request to access the `local` address space"). The scanner therefore probes
  through `network_probe_identity` (reqwest, no browser rules) whenever the bridge offers it;
  `fetch` stays for web and mobile.

The scanner logs the addresses it got, the subnets it chose, the transport and the first
transport error per subnet (`[network-scan]` console lines), and the "Servers on your network"
card shows the same summary ("Scanned N addresses via shell. Local addresses: … First
error: …") so a user can report what blocks it.
...
- `network_local_addresses`: `[{interface, ip, prefixLength}]`, the IPv4 addresses of
  ...
  `network_probe_identity {url}`: GET of a daemon's `/api/identity` or `/api/health` (only
  those paths, `http`/`https`, no query) with a 700 ms budget; answers `{status, body}` with
  the body parsed as JSON (`null` when it is not), or fails with the transport error
