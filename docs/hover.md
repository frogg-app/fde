...
The canonical implementation lives in `apps/ui/src/components/sidebar-workspace-list.tsx`, in the workspace row (around line 1369). When in doubt, open that file and copy the shape.
...
(`apps/ui/src/components/sidebar/use-open-kebab-menu-visibility.ts`) is that shape for the
...
For this case, use `useHoverSafeZone` (`apps/ui/src/hooks/use-hover-safe-zone.ts`). It computes a rectangular "bridge" between the trigger and the content; while the pointer is inside trigger, content, or the bridge, the card stays open. A short grace timer absorbs jitter at the edges. The canonical caller is `apps/ui/src/components/workspace-hover-card.tsx`.
