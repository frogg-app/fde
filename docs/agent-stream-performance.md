...
How assistant text gets from a provider to the screen, and why it is paced on the way. Read this before changing `packages/server/src/server/agent/agent-stream-coalescer.ts`, the reducer queue in `apps/ui/src/timeline/session-stream-reducers.ts`, or the reveal in `apps/ui/src/hooks/use-revealed-text.ts`.
