...
FDE's mobile route tree is fragile because Expo Router and React Navigation do
...
Read this before changing `apps/ui/src/app`, startup routing, remembered
...
- The root layout does not register host leaf routes such as
  ...
  `h/[serverId]/index`.
- `apps/ui/src/app/h/[serverId]/_layout.tsx` owns the host leaves with
...
Use `ThemedStack` from `apps/ui/src/navigation/themed-stack.tsx` for every
...
1. Seed `fde:last-workspace-route-selection` with a valid
...
- [ ] Did you change `apps/ui/src/app`? Re-read this file.
