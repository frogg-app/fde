...
const MEASUREMENT_CWD = "/tmp/fde-agent-tools-measurement";
...
Measures the MCP tools/list payload for Fde agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "fde-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Fde Agent Tool Catalog Context");
  lines.push("");
