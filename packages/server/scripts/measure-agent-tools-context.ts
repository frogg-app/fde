...
const MEASUREMENT_CWD = "/tmp/frogg-agent-tools-measurement";
...
Measures the MCP tools/list payload for Frogg agent tools.
...
async function measureCatalog(params: {
  ...
  const client = new Client({ name: "frogg-agent-tools-measurement", version: "0.0.0" });
...
function formatMarkdown(measurements: CatalogMeasurement[], top: number): string {
  ...
  const delta =
    withoutBrowser && withBrowser
      ...
      : null;
  const lines: string[] = [];
  lines.push("# Frogg Agent Tool Catalog Context");
  lines.push("");
