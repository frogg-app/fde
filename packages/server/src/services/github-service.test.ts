...
import { CheckoutPrStatusResponseSchema } from "@frogg/protocol/messages";
    ...
    data: {
      repository: {
        pullRequest: {
          ...
          headRepository: {
            sshUrl: "git@github.com:therainisme/frogg.git",
            url: "https://github.com/therainisme/frogg",
          },
        },
      },
    },
...
describe("ForgeService", () => {
  ...
  it("loads pull request checkout target details through GraphQL", async () => {
    ...
    expect(runner.calls[1]?.args).toContain("name=frogg");
      ...
      pullRequestTimelineJson({
        ...
        comments: {
          nodes: [
            {
              id: "IC_badge",
              body: "![build](https://img.shields.io/github/actions/workflow/status/frogg-app/frogg/ci.yml)",
              bodyHTML:
        ...
        statusCheckRollup: [
          {
            ...
            status: "IN_PROGRESS",
            conclusion: null,
            detailsUrl: "https://github.com/frogg-app/frogg/actions/runs/993",
          },
        ],
      }),
      currentPullRequestGithubFactsJson(),
      ...
      checks: [
        {
          name: "server tests",
          status: "pending",
          url: "https://github.com/frogg-app/frogg/actions/runs/993",
          workflow: "CI",
        },
      ],
  ...
  it("treats a GitHub issue or PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.com/frogg-app/frogg/pull/793",
      limit: 5,
    });
  ...
  it("does not treat an unrelated tracker URL as a GitHub issue/PR number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://gitlab.com/frogg-app/frogg/issues/793",
      limit: 5,
    });
    ...
    expect(runner.calls.map((call) => call.args)).toEqual([
      [
        ...
        "list",
        "--search",
        "https://gitlab.com/frogg-app/frogg/issues/793",
        "--json",
        "number,title,url,state,body,labels,updatedAt",
        "--limit",
        "5",
  ...
  it("treats a GitHub Enterprise issue/PR URL as a search for that number", async () => {
    ...
    await service.searchIssuesAndPrs({
      cwd: "/repo",
      query: "https://github.acme.internal/frogg-app/frogg/pull/793",
      limit: 5,
    });
