...
import type { FdeToolCatalog, FdeToolDefinition, FdeToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: FdeToolDefinition[]): FdeToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: FdeToolResult) => void) | null = null;
  ...
  static async withTools(tools: FdeToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: FdeToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Fde agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<FdeToolResult>((resolve) => {
  ...
  completeControlledCall(result: FdeToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, fdeTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped Fde tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create a Fde agent.",
