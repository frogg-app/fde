...
import type { FroggToolCatalog, FroggToolDefinition, FroggToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: FroggToolDefinition[]): FroggToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: FroggToolResult) => void) | null = null;
  ...
  static async withTools(tools: FroggToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: FroggToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Frogg agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<FroggToolResult>((resolve) => {
  ...
  completeControlledCall(result: FroggToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, froggTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped Frogg tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create a Frogg agent.",
