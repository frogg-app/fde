...
const CWD = "/tmp/fde-omp-agent-test";
...
async function writeOmpHistory(history: OmpResumeHistory): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "fde-omp-resume-"));
...
export class OmpHarness {
  ...
  async start(config: Partial<AgentSessionConfig> = {}, fdeTools?: FdeToolCatalog): Promise<void> {
    const session = await this.client.createSession(
      { provider: "omp", cwd: CWD, ...config },
      fdeTools ? { fdeTools } : undefined,
    );
