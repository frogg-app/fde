...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex frogg speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex frogg_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        ...
        server: "frogg_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via frogg_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via frogg_voice.",
      output: null,
    });
  });

  it("normalizes codex frogg speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
