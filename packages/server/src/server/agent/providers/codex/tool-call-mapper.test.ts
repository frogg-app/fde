...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex fde speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex fde_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        ...
        server: "fde_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via fde_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via fde_voice.",
      output: null,
    });
  });

  it("normalizes codex fde speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
