...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Frogg using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "frogg-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Frogg issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
