...
describe("opencode custom command Big Pickle E2E (real)", () => {
  ...
  test("executes a global custom command through Fde using Big Pickle", async () => {
    ...
    const projectDir = await mkdtemp(path.join(tmpdir(), "fde-opencode-big-pickle-"));
    ...
    try {
      ...
      await writeFile(
        ...
        [
          "---",
          "description: Fde issue 903 Big Pickle custom command",
          "agent: build",
          "---",
          "",
