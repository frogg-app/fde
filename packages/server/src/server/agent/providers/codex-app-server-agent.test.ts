...
async function startPublicSteeringSession(
...
): Promise<{ session: AgentSession; froggTurnId: string }> {
  const session = new CodexAppServerAgentSession(
  ...
  return { session, froggTurnId: started.turnId };
}
...
describe("Codex active-turn steering admission", () => {
  test("a steer without the clearing contract leaves permissions open", async () => {
    ...
    const { session, froggTurnId } = await startPublicSteeringSession(appServer);
    castInternals<{ emitSyntheticPlanApprovalRequest: (planText: string) => void }>(
      session,
    ).emitSyntheticPlanApprovalRequest("Ship the thing");

    await expect(
      session.steerActiveTurn!("background notification", { expectedTurnId: froggTurnId }),
  ...
  test("does not steer B when A completes while command resolution is pending", async () => {
    ...
    const appServer = createFakeCodexAppServer();
    const { session, froggTurnId } = await startPublicSteeringSession(appServer, async (prompt) => {
    ...
    expect(startedB.turnId).not.toBe(froggTurnId);
...
const capturePath = process.env.FROGG_FAKE_CODEX_CAPTURE;
let buffer = "";
    ...
    providerOverrides: {
      [providerId]: {
        ...
        env: {
          ...
          FROGG_FAKE_CODEX_CAPTURE: capturedRequestsPath,
        },
      },
    },
  });
...
describe("Codex app-server provider", () => {
  ...
  test("initializes Codex app-server without making Frogg the request originator", async () => {
  ...
  test("unarchives Codex when an active Frogg agent resumes an archived thread", async () => {
    const threadRequests: string[] = [];
  ...
  test("resolves Codex skill slash commands into app-server skill input", async () => {
          ...
          data: [
            {
              ...
              skills: [
                {
                  name: "frogg-implement",
                  description: "Execute an existing Frogg plan.",
                  path: "/tmp/skills/frogg-implement/SKILL.md",
                },
              ],
              errors: [],
            },
    ...
    await session.startTurn("/frogg-implement in a worktree, remember to use Claude for the UI");
    ...
    expect(turnStartCall?.[1]).toEqual(
      expect.objectContaining({
        input: [
          ...
          {
            type: "text",
            text: "$frogg-implement in a worktree, remember to use Claude for the UI",
            text_elements: [],
          },
        ],
      }),
    ...
    const commands = await listCommandsFromFakeCodex([
      {
        ...
        path: "/Users/test/.agents/skills/frogg/SKILL.md",
      },
      {
        name: "frogg",
        description: "Shared orchestration skill.",
        path: "/Users/test/.codex/skills/frogg/SKILL.md",
      },
    ]);

    expect(commands.filter((command) => command.name === "frogg")).toEqual([
      {
        name: "frogg",
        description: "Shared orchestration skill.",
        argumentHint: "",
    ...
    const input = await codexAppServerTurnInputFromPrompt(
      [
        { type: "text", text: "Please review this" },
        {
          ...
          url: "https://github.com/frogg-app/frogg/issues/456",
        },
      ],
      logger,
    );
  ...
  test("builds app-server env from launch-context env overrides", () => {
    const launchContext: AgentLaunchContext = {
      env: {
        FROGG_AGENT_ID: "00000000-0000-4000-8000-000000000301",
        FROGG_TEST_FLAG: "codex-launch-value",
      },
    };
    const env = buildCodexAppServerEnv(
      {
        env: {
          FROGG_AGENT_ID: "runtime-value",
          FROGG_TEST_FLAG: "runtime-test-value",
        },
      },
      launchContext.env,
    );

    expect(env.FROGG_AGENT_ID).toBe(launchContext.env?.FROGG_AGENT_ID);
    expect(env.FROGG_TEST_FLAG).toBe(launchContext.env?.FROGG_TEST_FLAG);
  });
    ...
    expect(events).toEqual([
      {
        ...
        item: {
          type: "assistant_message",
          text: "![Image](file:///tmp/frogg%20image.png)",
        },
      },
    ]);
  });
  ...
  test("materializes imageGeneration base64 results before rendering markdown", () => {
    ...
    expect(source).toMatch(/frogg-attachments(?:-[^\\/]+)?[\\/].+\.png$/);
    expect(existsSync(source)).toBe(true);
    rmSync(source, { force: true });
  });
