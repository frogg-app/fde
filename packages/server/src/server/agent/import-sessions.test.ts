...
import type { FetchRecentProviderSessionsRequestMessage } from "@frogg/protocol/messages";
...
test("listImportableProviderSessions includes a provider session after its Frogg agent is archived", async () => {
  const cwd = "/tmp/project";
...
test("listImportableProviderSessions keeps realpath-equivalent cwd matches", async () => {
  const root = mkdtempSync(path.join(tmpdir(), "frogg-import-cwd-"));
