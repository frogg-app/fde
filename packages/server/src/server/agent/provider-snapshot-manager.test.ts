...
describe("ProviderSnapshotManager public surface", () => {
  ...
  test("FROGG_PROVIDER_REFRESH_TIMEOUT_MS env var is honored when no option is given", async () => {
    vi.stubEnv("FROGG_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(waitUntilAborted);
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      providerOverrides: {
  ...
  test("FROGG_PROVIDER_REFRESH_TIMEOUT_MS env var is ignored when option is provided", async () => {
    vi.stubEnv("FROGG_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(waitUntilAborted);
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
  ...
  test("getProviderDiagnostic reports a stuck catalog refresh inside the diagnostic", async () => {
    await withEnv("FROGG_ENABLE_MOCK_SLOW", "true", async () => {
      vi.useFakeTimers();
      const manager = new ProviderSnapshotManager({
        logger: createTestLogger(),
        isDev: true,
