import type { DaemonTransportFactory } from "@fde/client/internal/daemon-client-transport-types";
...
export function createPluginDaemonTransportFactory(
  ...
  return () => {
    ...
    const unsubscribe = port.onMessage((message) => {
      ...
      if (message.type === "fde_close") closeTransport();
    });
    ...
    return {
      send(data) {
        ...
        port.send({ type: "fde_frame", data: frame, isBinary: typeof frame !== "string" });
      },
      close() {
        if (!isOpen) return;
        port.send({ type: "fde_close" });
        closeTransport();
      },
      onMessage(handler) {
