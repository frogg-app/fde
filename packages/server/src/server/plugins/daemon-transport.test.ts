...
describe("plugin daemon IPC transport", () => {
  it("preserves incoming text and binary frame order", async () => {
    ...
    await opened;
    receive?.({ type: "fde_frame", data: "first", isBinary: false });
    receive?.({ type: "fde_frame", data: new Uint8Array([7, 8]), isBinary: true });
    ...
    expect(sent).toEqual([{ type: "fde_frame", data: "outbound", isBinary: false }]);
  });
});
