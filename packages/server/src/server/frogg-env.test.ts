...
describe("frogg env contract", () => {
  ...
  const baseEnv = {
    ...
    PATH: "/usr/bin",
    FROGG_AGENT_ID: "agent-123",
    FROGG_DESKTOP_MANAGED: "1",
    [FROGG_NODE_ENV]: "production",
    FROGG_SUPERVISED: "1",
    ESBUILD_BINARY_PATH: "/Applications/Frogg.app/Contents/Resources/app.asar.unpacked/esbuild",
  };
  ...
  test("builds internal daemon child env by preserving pass-through and control vars", () => {
    const env = createFroggInternalEnv(baseEnv);
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.FROGG_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.FROGG_DESKTOP_MANAGED).toBeUndefined();
    expect(env[FROGG_NODE_ENV]).toBeUndefined();
    expect(env.FROGG_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.FROGG_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[FROGG_NODE_ENV]).toBeUndefined();
    expect(command.env.FROGG_SUPERVISED).toBeUndefined();
  });
  ...
  test("does not use user NODE_ENV as Frogg runtime mode", () => {
    expect(resolveFroggNodeEnv({ NODE_ENV: "development" })).toBeUndefined();
    expect(resolveFroggNodeEnv({ NODE_ENV: "development", FROGG_NODE_ENV: "production" })).toBe(
      "production",
    );
