  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$FDE_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.FDE_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.FDE_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalWorkspaceIds).toEqual(["ws-shared-runtime-port"]);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "fde_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.FDE_SERVICE_API_URL).toBe("http://api--feature-peer-env--repo.localhost:9999");
      expect(apiEnv.FDE_SERVICE_WEB_URL).toBe("http://web--feature-peer-env--repo.localhost:9999");
      expect(apiEnv.FDE_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.FDE_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.FDE_URL).toBe(apiEnv.FDE_SERVICE_API_URL);
      expect(apiEnv.FDE_PORT).toBe(apiEnv.FDE_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.FDE_SERVICE_API_URL).toBe("http://api--feature-peer-env--repo.localhost:9999");
      expect(webEnv.FDE_SERVICE_WEB_URL).toBe("http://web--feature-peer-env--repo.localhost:9999");
      expect(webEnv.FDE_SERVICE_API_PORT).toBe(apiEnv.FDE_SERVICE_API_PORT);
      expect(webEnv.FDE_SERVICE_WEB_PORT).toBe(apiEnv.FDE_SERVICE_WEB_PORT);
      expect(webEnv.FDE_URL).toBe(webEnv.FDE_SERVICE_WEB_URL);
      expect(webEnv.FDE_PORT).toBe(webEnv.FDE_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.FDE_SERVICE_API_PORT);
      const webPort = Number(apiEnv.FDE_SERVICE_WEB_PORT);
