  ...
  describe("runAsyncWorktreeBootstrap", () => {
    ...
    it("shares the same worktree runtime port across setup and bootstrap terminals", async () => {
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: ['echo "$FROGG_WORKTREE_PORT" > setup-port.txt'],
            terminals: [
              {
                name: "Port Terminal",
                command: "true",
      ...
      expect(registeredEnvs[0]?.env.FROGG_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalEnvs.length).toBeGreaterThan(0);
      expect(createTerminalEnvs[0]?.FROGG_WORKTREE_PORT).toBe(setupPort);
      expect(createTerminalWorkspaceIds).toEqual(["ws-shared-runtime-port"]);

      const terminalToolCall = persisted.find(
        (item): item is Extract<AgentTimelineItem, { type: "tool_call" }> =>
          item.type === "tool_call" &&
          item.name === "frogg_worktree_terminals" &&
    ...
    it("injects real peer service env into terminal-backed services", async () => {
      ...
      expect(apiEnv.FROGG_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:9999",
      );
      expect(apiEnv.FROGG_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:9999",
      );
      expect(apiEnv.FROGG_SERVICE_API_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.FROGG_SERVICE_WEB_PORT).toEqual(expect.stringMatching(/^\d+$/));
      expect(apiEnv.FROGG_URL).toBe(apiEnv.FROGG_SERVICE_API_URL);
      expect(apiEnv.FROGG_PORT).toBe(apiEnv.FROGG_SERVICE_API_PORT);
      expect(apiEnv).not.toHaveProperty("PORT");

      expect(webEnv.FROGG_SERVICE_API_URL).toBe(
        "http://api--feature-peer-env--repo.localhost:9999",
      );
      expect(webEnv.FROGG_SERVICE_WEB_URL).toBe(
        "http://web--feature-peer-env--repo.localhost:9999",
      );
      expect(webEnv.FROGG_SERVICE_API_PORT).toBe(apiEnv.FROGG_SERVICE_API_PORT);
      expect(webEnv.FROGG_SERVICE_WEB_PORT).toBe(apiEnv.FROGG_SERVICE_WEB_PORT);
      expect(webEnv.FROGG_URL).toBe(webEnv.FROGG_SERVICE_WEB_URL);
      expect(webEnv.FROGG_PORT).toBe(webEnv.FROGG_SERVICE_WEB_PORT);
      expect(webEnv).not.toHaveProperty("PORT");

      const apiPort = Number(apiEnv.FROGG_SERVICE_API_PORT);
      const webPort = Number(apiEnv.FROGG_SERVICE_WEB_PORT);
