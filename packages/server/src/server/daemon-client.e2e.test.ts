  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.fdeHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "fde-dir-suggestion-"));
  const rootBrowseDir = mkdtempSync(path.join(homedir(), "000-fde-root-browse-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "fde-dir-suggestion-outside-"));
...
test("returns typed relative suggestions within a requested directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "fde-workspace-suggestion-"));
...
test("finds workspace files inside the OpenCode directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "fde-opencode-suggestion-"));
...
test("opens an exact gitignored workspace path without offering it as a suggestion", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "fde-gitignored-suggestion-"));
...
test("a Desktop-managed daemon does not advertise npm self-update", async () => {
  const daemon = await createTestFdeDaemon({ desktopManaged: true });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
