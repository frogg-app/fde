...
const localModelsDir =
  process.env.FROGG_LOCAL_MODELS_DIR ?? path.join(homedir(), ".frogg", "models", "local-speech");
const testFileDir = path.dirname(fileURLToPath(import.meta.url));
const appE2eFixturesDir = path.resolve(testFileDir, "../../../../apps/ui/e2e/support/fixtures");
  ...
  try {
    ...
    expect(result).toEqual({
      ...
      file: {
        ...
        mimeType: "text/plain",
        size: 11,
        path: path.join(daemon.froggHome, "uploads", "upload_req-upload-e2e", "notes.txt"),
      },
      error: null,
    });
...
test("returns home-scoped directory suggestions", async () => {
  const insideHomeDir = mkdtempSync(path.join(homedir(), "frogg-dir-suggestion-"));
  const rootBrowseDir = mkdtempSync(path.join(homedir(), "000-frogg-root-browse-"));
  const outsideHomeDir = mkdtempSync(path.join(tmpdir(), "frogg-dir-suggestion-outside-"));
...
test("returns typed relative suggestions within a requested directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-workspace-suggestion-"));
...
test("finds workspace files inside the OpenCode directory", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-opencode-suggestion-"));
...
test("opens an exact gitignored workspace path without offering it as a suggestion", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-gitignored-suggestion-"));
...
test("a Desktop-managed daemon does not advertise npm self-update", async () => {
  const daemon = await createTestFroggDaemon({ desktopManaged: true });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
