...
import type { AgentSnapshotPayload, SessionOutboundMessage } from "@frogg/protocol/messages";
...
function createSessionForWorkspaceTests(
  ...
  const session = asTestSession(
    new Session({
      ...
      froggHome: options.froggHome ?? "/tmp/frogg-test",
...
test("create_agent_request keeps requested child cwd when grouped under an existing parent workspace", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "frogg-create-agent-cwd-"));
  try {
    ...
    const session = asTestSession(
      new Session({
        ...
        downloadTokenStore: asDownloadTokenStore(),
        pushNotifications: asPushNotifications(),
        froggHome: path.join(workdir, "frogg-home"),
        agentManager,
        agentStorage,
        projectRegistry,
        workspaceRegistry,
...
test("create_agent_request launches from an exact subdirectory in a created worktree", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "frogg-create-agent-worktree-cwd-"));
  try {
    const parent = path.join(workdir, "parent");
    ...
    execFileSync("git", ["config", "user.name", "Frogg Test"], { cwd: parent, stdio: "pipe" });
...
test("create_agent_request does not title an existing workspace from the agent prompt", async () => {
  vi.useFakeTimers();
  const workdir = mkdtempSync(path.join(tmpdir(), "frogg-create-agent-existing-title-"));
  try {
...
test("workspace placements preserve checkout facts independently from the project", async () => {
  ...
  const froggSubdirectory = createPersistedWorkspaceRecord({
    workspaceId: "ws-frogg-subdirectory",
    projectId: "proj-manual-worktree",
    cwd: "/tmp/frogg-worktree/packages/app",
    kind: "worktree",
    displayName: "app",
  ...
  await expect(
    session.buildProjectPlacementForWorkspaceId(froggSubdirectory.workspaceId),
  ).resolves.toEqual(
    expect.objectContaining({
      checkout: expect.objectContaining({
        cwd: froggSubdirectory.cwd,
        worktreeRoot: "/tmp/frogg-worktree",
      }),
    }),
  );
});
...
test("create frogg worktree response preserves an explicit non-Git project", async () => {
  const emitted: SessionOutboundMessage[] = [];
  ...
  try {
    await session.handleCreateFroggWorktreeRequest({
  ...
  const response = findByType(emitted, "create_frogg_worktree_response");
...
test("workspace updates stay scoped to the matching cwd", async () => {
  ...
  const missingRoot = path.join(tmpdir(), `frogg-scoped-workspace-${Date.now()}`);
...
test("open_project_request does not match a new child directory to an existing parent workspace", async () => {
  ...
  const worktree = path.join(home, ".frogg", "worktrees", "project-config-lifecycle-textarea");
...
test("open_project_request reclassifies an archived directory workspace when git metadata becomes available", async () => {
  ...
  const projects = new Map<string, ReturnType<typeof createPersistedProjectRecord>>();
  const workspaces = new Map<string, ReturnType<typeof createPersistedWorkspaceRecord>>();
  const repoRoot = path.resolve("/home/developer/dev/frogg");
  const cwd = path.join(
...
test("open_project_request reclassifies an active directory workspace when git metadata becomes available", async () => {
  ...
  const repoWorkspaceId = "ws-frogg-main";
  workspaces.set(
    workspaceId,
    createPersistedWorkspaceRecord({
      workspaceId,
...
test("refresh_agent_request leaves workspace archival independent when its directory exists", async () => {
  ...
  const cwd = path.resolve("/tmp/frogg-unit2-existing-dir");
  session.filesystem.isDirectory = async () => true;
  const workspaceId = "ws-repo-archived";
...
test("refresh_agent_request leaves workspace archival independent when its directory is missing", async () => {
  ...
  const cwd = path.resolve("/tmp/frogg-missing-workspace-dir");
...
test("refresh_agent_request does not recreate or unarchive a deleted worktree", async () => {
  ...
  const cwd = path.resolve("/tmp/frogg-deleted-worktree-dir");
  session.filesystem.isDirectory = async () => false;
...
test("refresh_agent_request does not inspect an archived worktree branch", async () => {
  ...
  const cwd = path.resolve("/tmp/frogg-deleted-worktree-fail");
  session.filesystem.isDirectory = async () => false;
...
test.skip("opening a new worktree reconciles older local workspaces into the remote project", async () => {
  ...
  const worktreeWorkspaceId = path.join(mainWorkspaceId, ".frogg", "worktrees", "feature-a");
  ...
  session.buildProjectPlacement = async (cwd: string) => ({
    ...
    checkout: {
      ...
      worktreeRoot: cwd,
      isFroggOwnedWorktree: cwd !== mainWorkspaceId,
...
test("title-only terminal change does not build workspace descriptors or emit workspace_update", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-session-title-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("terminal activity contribution change updates the correct workspace", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-session-activity-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("same-cwd terminal activity updates only the workspace that owns the terminal", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-session-same-cwd-"));
  const workspaceA = createPersistedWorkspaceRecord({
...
test("a worktree terminal updates only the workspace that owns it", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const rootCwd = mkdtempSync(path.join(tmpdir(), "frogg-session-nested-"));
...
test("removing an idle terminal does not update workspace status", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-session-remove-idle-"));
  const workspace = createPersistedWorkspaceRecord({
...
test("removing a contributing terminal clears workspace status", async () => {
  const emitted: SessionOutboundMessage[] = [];
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-session-remove-contrib-"));
  const workspace = createPersistedWorkspaceRecord({
...
function createWorkspaceCreatePrRepo(): WorkspaceCreatePrRepoFixture {
  ...
  return { tempDir, repoDir, froggHome, headRef, prFileName, prNumber };
}
...
test("workspace auto-name uses the backing root for a nested worktree", async () => {
  ...
  writeFroggWorktreeMetadata(repoDir, { baseRefName: "main" });
  writeFroggWorktreeFirstAgentBranchAutoNameMetadata(repoDir, {
  ...
  try {
    ...
    expect(readFroggWorktreeMetadata(repoDir)).toMatchObject({
      version: 2,
