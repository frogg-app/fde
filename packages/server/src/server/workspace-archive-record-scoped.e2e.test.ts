...
// derived from scope + last-reference + FDE ownership; there is no caller-
// supplied disk flag.
...
test("archiving the last reference to a worktree removes it from disk regardless of the disk flag", async () => {
  ...
  const keepArchive = await ctx.client.archiveFdeWorktree({ worktreePath: keepDir });
  ...
  const deleteArchive = await ctx.client.archiveFdeWorktree({ worktreePath: deleteDir });
...
test("worktree archive targets the explicit workspaceId when a directory backs multiple workspaces", async () => {
  ...
  await ctx.client.archiveFdeWorktree({
    worktreePath: worktreeDir,
