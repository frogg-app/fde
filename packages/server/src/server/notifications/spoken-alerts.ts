...
export function isSpokenNotificationsEnabled(speech: FroggSpeechConfig | undefined): boolean {
...
export function createSpokenAlertService(deps: SpokenAlertServiceDeps): SpokenAlertService {
  ...
  return {
    ...
    async read(id) {
      ...
      // Another reader may have started a retry while the cache read was pending.
      const retryInFlight = inFlight.get(id);
