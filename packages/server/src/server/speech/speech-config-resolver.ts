...
import type { FroggOpenAIConfig, FroggSpeechConfig } from "../bootstrap.js";
 ...
 *
 * 1. `FROGG_VOICE=0` / `features.voice.enabled=false` turns everything off
 * 2. the fine-grained key (`FROGG_DICTATION_ENABLED`, `features.dictation.enabled`, ...)
 * 3. `FROGG_VOICE=1` / `features.voice.enabled=true`
...
function buildFeatureProviderInputs(params: {
  ...
  const voiceModeEnabled = resolveVoiceFeatureEnabled(
    parseOptionalBooleanFlag(
      firstSpeechDefinedValue<string | boolean>([
        params.env.FROGG_VOICE_MODE_ENABLED,
  ...
  return {
    ...
    voiceTurnDetection: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.FROGG_VOICE_TURN_DETECTION_PROVIDER,
    ...
    voiceStt: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.FROGG_VOICE_STT_PROVIDER,
    ...
    voiceTts: {
      configuredValue: firstSpeechDefinedValue<string>([
        params.env.FROGG_VOICE_TTS_PROVIDER,
...
function resolveVoiceNotificationsEnabled(params: {
  ...
  return resolveVoiceFeatureEnabled(
    parseOptionalBooleanFlag(
      firstSpeechDefinedValue<string | boolean>([
        params.env.FROGG_VOICE_NOTIFICATIONS,
...
/** The daemon enables Companion by default; clients opt in separately.
 * Missing models/authentication are readiness failures, not administrative disablement.
 */
export function resolveCompanionFeatureEnabled(params: {
  ...
  if (umbrella === false) return false;
  return (
    parseOptionalBooleanFlag(
      firstSpeechDefinedValue<string | boolean>([
        params.env.FROGG_COMPANION_ENABLED,
...
export function resolveSpeechConfig(params: {
  ...
  openai: FroggOpenAIConfig | undefined;
