...
const RUN = process.env.FROGG_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const froggHomeRoot = mkdtempSync(path.join(tmpdir(), "frogg-speech-download-"));
    const modelsDir = path.join(froggHomeRoot, ".frogg", "models", "local-speech");
