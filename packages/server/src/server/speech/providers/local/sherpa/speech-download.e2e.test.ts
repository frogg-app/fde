...
const RUN = process.env.FDE_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const fdeHomeRoot = mkdtempSync(path.join(tmpdir(), "fde-speech-download-"));
    const modelsDir = path.join(fdeHomeRoot, ".fde", "models", "local-speech");
