...
function resolveFilePath(fdeHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(fdeHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(fdeHome, configuredPath);
}
...
function resolveConfiguredFdeHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.fdeHome) {
    return options.fdeHome;
  }
  return resolveFdeHome();
}
...
export function resolveLogConfig(
  ...
  const fdeHome = resolveConfiguredFdeHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(fdeHome, persistedLog.file.path),
        }
      : undefined;
