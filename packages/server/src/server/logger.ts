...
function resolveFilePath(froggHome: string, configuredPath: string | undefined): string {
  const fallback = path.join(froggHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (!configuredPath) {
    return fallback;
  }
  ...
  return path.resolve(froggHome, configuredPath);
}
...
function resolveConfiguredFroggHome(options: ResolveLogConfigOptions | undefined): string {
  if (options?.froggHome) {
    return options.froggHome;
  }
  return resolveFroggHome();
}
...
export function resolveLogConfig(
  ...
  const froggHome = resolveConfiguredFroggHome(options);
  ...
  const file =
    options?.file !== false && persistedLog?.file
      ? {
          ...
          path: resolveFilePath(froggHome, persistedLog.file.path),
        }
      : undefined;
