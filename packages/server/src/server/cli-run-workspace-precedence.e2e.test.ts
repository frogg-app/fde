...
// The daemon-level workspace contract that `fde run` depends on: each
...
// precedence (--workspace > $FDE_WORKSPACE_ID > --worktree > bare) is covered
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "fde-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "fde-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $FDE_WORKSPACE_ID land a run) creates no new workspace record: the
