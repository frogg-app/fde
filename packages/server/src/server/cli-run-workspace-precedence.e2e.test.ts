...
// The daemon-level workspace contract that `frogg run` depends on: each
...
// precedence (--workspace > $FROGG_WORKSPACE_ID > --worktree > bare) is covered
// in apps/cli/src/commands/agent/run.test.ts; this test only proves the
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "frogg-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "frogg-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $FROGG_WORKSPACE_ID land a run) creates no new workspace record: the
