...
import type { AgentStreamEvent } from "@frogg/protocol/agent-types";
import type { HubExecutionAgentCreateResponse } from "@frogg/protocol/messages";
...
const RUN_REAL_HUB_POLICY = process.env.FROGG_REAL_HUB_POLICY === "1";
...
describe.skipIf(!RUN_REAL_HUB_POLICY)("Hub provider policy (real providers)", () => {
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "frogg-codex-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "frogg-codex-denied-"));
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "frogg-claude-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "frogg-claude-denied-"));
      const allowedFile = path.join(allowedRoot, "allowed.txt");
      const deniedFile = path.join(deniedRoot, "denied.txt");
      const providerOptions = {
