...
import type { AgentStreamEvent } from "@fde/protocol/agent-types";
import type { HubExecutionAgentCreateResponse } from "@fde/protocol/messages";
...
const RUN_REAL_HUB_POLICY = process.env.FDE_REAL_HUB_POLICY === "1";
...
describe.skipIf(!RUN_REAL_HUB_POLICY)("Hub provider policy (real providers)", () => {
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "fde-codex-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "fde-codex-denied-"));
  ...
  test(
    ...
    async () => {
      const sink = await startHubActionSink();
      const allowedRoot = await mkdtemp(path.join(tmpdir(), "fde-claude-allowed-"));
      const deniedRoot = await mkdtemp(path.join(tmpdir(), "fde-claude-denied-"));
      const allowedFile = path.join(allowedRoot, "allowed.txt");
      const deniedFile = path.join(deniedRoot, "denied.txt");
      const providerOptions = {
