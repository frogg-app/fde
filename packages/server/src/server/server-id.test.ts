...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "frogg-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.FROGG_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per FROGG_HOME", () => {
  ...
  it("respects and persists FROGG_SERVER_ID override", () => {
    process.env.FROGG_SERVER_ID = "test-daemon-id";
