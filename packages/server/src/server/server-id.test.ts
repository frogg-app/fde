...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "fde-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.FDE_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per FDE_HOME", () => {
  ...
  it("respects and persists FDE_SERVER_ID override", () => {
    process.env.FDE_SERVER_ID = "test-daemon-id";
