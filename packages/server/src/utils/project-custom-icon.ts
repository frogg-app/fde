...
export async function setProjectCustomIcon(input: {
  ...
  return serialize(input.projectId, async () => {
    ...
    if (input.source.type === "automatic") {
      ...
      validateIcon(bytes);
      await writeFileAtomic(cachePath(input.froggHome, input.projectId), bytes);
...
export class ProjectIconReader {
  ...
  async snapshot(project: PersistedProjectRecord): Promise<ProjectIconSnapshot> {
    const snapshot = await readProjectIconSnapshot({ froggHome: this.froggHome, project });
...
export async function readProjectIconSnapshot(input: {
  ...
  if (input.project.customIconRevision) {
    ...
    try {
      icon = validateIcon(await readFile(cachePath(input.froggHome, input.project.projectId)));
    } catch {
...
export async function removeProjectCustomIcon(input: {
  ...
  await rm(cachePath(input.froggHome, input.projectId), { force: true });
}
...
function cachePath(froggHome: string, projectId: string): string {
  const key = createHash("sha256").update(projectId).digest("hex");
  return join(froggHome, "projects", "icons", `${key}.bin`);
}
