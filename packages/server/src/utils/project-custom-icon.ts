...
export async function setProjectCustomIcon(input: {
  ...
  return serialize(input.projectId, async () => {
    ...
    if (input.source.type === "automatic") {
      ...
      validateIcon(bytes);
      await writeFileAtomic(cachePath(input.fdeHome, input.projectId), bytes);
...
export class ProjectIconReader {
  ...
  constructor(private readonly fdeHome: string) {}

  async snapshot(project: PersistedProjectRecord): Promise<ProjectIconSnapshot> {
    const snapshot = await readProjectIconSnapshot({ fdeHome: this.fdeHome, project });
...
export async function readProjectIconSnapshot(input: {
  ...
  if (input.project.customIconRevision) {
    ...
    try {
      icon = validateIcon(await readFile(cachePath(input.fdeHome, input.project.projectId)));
    } catch {
...
export async function removeProjectCustomIcon(input: {
  ...
  await rm(cachePath(input.fdeHome, input.projectId), { force: true });
}
...
function cachePath(fdeHome: string, projectId: string): string {
  const key = createHash("sha256").update(projectId).digest("hex");
  return join(fdeHome, "projects", "icons", `${key}.bin`);
}
