...
export const OPENCODE_PLUGIN_SOURCE = [
  ...
  "}",
  "",
  "function runFroggHook(event) {",
  "  if (!process.env.FROGG_TERMINAL_ID) return;",
  "  try {",
  '    const child = Bun.spawn(["frogg", "hooks", "opencode", event], {',
  '      stdin: "ignore",',
  '      stdout: "ignore",',
  '      stderr: "ignore",',
  "    });",
  ...
  "  event: async ({ event }) => {",
  "    const froggEvent = froggEventFor(event);",
  "    if (froggEvent) runFroggHook(froggEvent);",
  "  },",
  "});",
  "",
].join("\n");
...
export function createOpenCodePluginInstallStrategy(): AgentHookPluginFileInstallStrategy {
  return {
    ...
    configFile: "plugins/frogg-terminal-activity.js",
    configDirEnvOverride: "OPENCODE_CONFIG_DIR",
    hookMarker: "frogg hooks opencode",
