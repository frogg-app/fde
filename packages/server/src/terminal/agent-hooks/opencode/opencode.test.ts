...
describe("OpenCode terminal agent hooks", () => {
  it("installs a self-contained OpenCode plugin idempotently", () => {
    const configDir = createTempDir("fde-opencode-config-");
    ...
    expect(firstInstall.configPath).toBe(join(configDir, "plugins", "fde-terminal-activity.js"));
  ...
  it("writes the plugin that maps OpenCode bus events to fde hook events", () => {
    const configDir = createTempDir("fde-opencode-config-source-");
    ...
    expect(source).toContain('Bun.spawn(["fde", "hooks", "opencode", event]');
    expect(source).toContain("FDE_TERMINAL_ID");
  });

  it("uninstalls the OpenCode plugin file", () => {
    const configDir = createTempDir("fde-opencode-config-uninstall-");
  ...
  it("prefers OPENCODE_CONFIG_DIR over the XDG config home", () => {
    const homeDir = createTempDir("fde-home-");
    const configDir = createTempDir("fde-opencode-override-");
    const xdgConfigHome = createTempDir("fde-xdg-config-");
    ...
    expect(configPath).toBe(join(configDir, "plugins", "fde-terminal-activity.js"));
  });
  ...
  it("uses the XDG config home for the default OpenCode config dir", () => {
    ...
    expect(configPath).toBe(join(xdgConfigHome, "opencode", "plugins", "fde-terminal-activity.js"));
  });
  ...
  it("falls back to the home .config OpenCode dir without an XDG config home", () => {
    ...
    expect(configPath).toBe(
      join(homeDir, ".config", "opencode", "plugins", "fde-terminal-activity.js"),
    );
  });
