...
describe("OpenCode terminal agent hooks", () => {
  it("installs a self-contained OpenCode plugin idempotently", () => {
    const configDir = createTempDir("frogg-opencode-config-");
    ...
    expect(firstInstall.configPath).toBe(join(configDir, "plugins", "frogg-terminal-activity.js"));
  ...
  it("writes the plugin that maps OpenCode bus events to frogg hook events", () => {
    const configDir = createTempDir("frogg-opencode-config-source-");
    ...
    expect(source).toContain('Bun.spawn(["frogg", "hooks", "opencode", event]');
    expect(source).toContain("FROGG_TERMINAL_ID");
  });

  it("uninstalls the OpenCode plugin file", () => {
    const configDir = createTempDir("frogg-opencode-config-uninstall-");
  ...
  it("prefers OPENCODE_CONFIG_DIR over the XDG config home", () => {
    const homeDir = createTempDir("frogg-home-");
    const configDir = createTempDir("frogg-opencode-override-");
    const xdgConfigHome = createTempDir("frogg-xdg-config-");
    ...
    expect(configPath).toBe(join(configDir, "plugins", "frogg-terminal-activity.js"));
  });
  ...
  it("uses the XDG config home for the default OpenCode config dir", () => {
    ...
    expect(configPath).toBe(
      join(xdgConfigHome, "opencode", "plugins", "frogg-terminal-activity.js"),
    );
  });
  ...
  it("falls back to the home .config OpenCode dir without an XDG config home", () => {
    ...
    expect(configPath).toBe(
      join(homeDir, ".config", "opencode", "plugins", "frogg-terminal-activity.js"),
    );
  });
