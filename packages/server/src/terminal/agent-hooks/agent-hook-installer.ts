...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${FROGG_HOOK_CLI:-frogg}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$FROGG_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined FROGG_TERMINAL_ID (if defined FROGG_HOOK_CLI ("%FROGG_HOOK_CLI%" ${hookArgs}) else (frogg ${hookArgs})) else (exit /b 0)`;
}
