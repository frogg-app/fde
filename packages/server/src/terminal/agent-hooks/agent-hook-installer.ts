...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${FDE_HOOK_CLI:-fde}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$FDE_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined FDE_TERMINAL_ID (if defined FDE_HOOK_CLI ("%FDE_HOOK_CLI%" ${hookArgs}) else (fde ${hookArgs})) else (exit /b 0)`;
}
