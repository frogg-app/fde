...
describe("plugin selector shallow equality", () => {
  it("keeps equivalent selector objects and arrays stable", () => {
    ...
    expect(shallowEqual({ name: "Fde", shared }, { name: "Fde", shared })).toBe(true);
    expect(shallowEqual(["Fde", shared], ["Fde", shared])).toBe(true);
    expect(shallowEqual({ name: "Fde" }, { name: "Changed" })).toBe(false);
  });
