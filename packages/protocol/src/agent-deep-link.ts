...
export function buildAgentDeepLink(target: AgentDeepLinkTarget, scheme = "frogg"): string {
  return `${scheme}:/${buildAgentDeepLinkRoute(target)}`;
}

export function parseAgentDeepLink(input: string, scheme = "frogg"): AgentDeepLinkTarget | null {
  let url: URL;
  try {
    url = new URL(input);
  } catch {
