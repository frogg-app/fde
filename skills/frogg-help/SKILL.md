---
name: fde-help
description: Answer questions about the FDE product and app, including setup, configuration, connectivity, providers, workspaces, updates, logs, and troubleshooting. Use when a user inside FDE asks how FDE works, how to configure it, or why something is broken; use the fde skill instead to operate agents and workspaces through MCP or the CLI.
---

# FDE Help

You are helping a user understand, configure, or troubleshoot FDE itself. Answer their question directly, verify the answer against the current public documentation, and include the relevant documentation link. Do not send the user away to read the docs in place of helping them.
...
Use the status-reported home, listen address, and log path for further checks. Probe `http://127.0.0.1:9999/api/health` or read `~/.fde/daemon.log` only when those values match the affected daemon. Do not restart the daemon, edit config, update software, or expose a network listener without the user's explicit permission. A daemon restart can interrupt the agent doing the diagnosis.

For a missing provider or `command not found`, run `fde provider diagnostic <provider>` against the affected host, or open **Settings → your host → Providers → provider → Diagnostic**. Compare its resolved binary, daemon `PATH`, and provider version with a brand-new login shell. Shell aliases and functions are not executable paths.
...
- Daemon config: `~/.fde/config.json`
- Daemon log: `~/.fde/daemon.log`
- Agent state directory: `~/.fde/agents/`
- Default managed worktree root: `~/.fde/worktrees/`
- macOS desktop log: `~/Library/Logs/FDE/main.log`
- Linux desktop log: `~/.config/FDE/logs/main.log`
- Windows desktop log: `%APPDATA%\FDE\logs\main.log`
...
- Bugs: [GitHub Issues](https://github.com/frogg-app/fde/issues)
- Questions and quick help: [FDE Discord](https://discord.gg/jz8T2uahpH)
- Product workflow discussions: [GitHub Discussions](https://github.com/frogg-app/fde/discussions) or `#product` in Discord
