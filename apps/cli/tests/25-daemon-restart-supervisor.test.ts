...
const fdeHome = await mkdtemp(join(tmpdir(), "fde-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(fdeHome);
  ...
  const statusAfterRestart = await readDaemonStatus(fdeHome);
  assert.strictEqual(
