...
/**
 * Test runner for Frogg CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp FROGG_HOME), so parallelism is safe.
 */
...
// packages live in the root. Prepend it so `npx frogg` resolves locally.
...
const concurrencyEnv = process.env.FROGG_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.FROGG_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.FROGG_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `FROGG_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@frogg/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Frogg CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "frogg-cli-test-npm-cache-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          FROGG_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.FROGG_LOCAL_SPEECH_AUTO_DOWNLOAD,
          FROGG_DICTATION_ENABLED: testEnvDefaults.FROGG_DICTATION_ENABLED,
          FROGG_VOICE_MODE_ENABLED: testEnvDefaults.FROGG_VOICE_MODE_ENABLED,
        },
