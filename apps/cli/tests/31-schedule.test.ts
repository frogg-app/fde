...
try {
  {
    ...
    const listed = await ctx.frogg(["schedule", "ls", "--json"]);
    ...
    const inspected = await ctx.frogg(["schedule", "inspect", createdJson.id, "--json"]);
  ...
  await ctx.stop();
  await rm(ctx.froggHome, { recursive: true, force: true });
