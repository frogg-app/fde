 ...
 *
 * Tests daemon commands with an isolated FROGG_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalFrogg(["daemon", ...args], { FROGG_HOME: froggHome });
}
...
function resolveDaemonWorkerEntry(): string {
  let currentDir = dirname(require.resolve("@frogg/server"));

  while (true) {
    const packageJsonPath = join(currentDir, "package.json");
    if (existsSync(packageJsonPath)) {
      const packageJson = JSON.parse(readFileSync(packageJsonPath, "utf-8"));
      if (packageJson.name === "@frogg/server") {
        const candidates = [
  ...
  throw new Error("Unable to resolve @frogg/server package root");
}

async function tailDaemonLog(): Promise<string> {
  try {
    const log = await readFile(join(froggHome, "daemon.log"), "utf-8");
    return log.split("\n").slice(-30).join("\n");
  } catch {
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalFrogg(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, froggHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
  ...
  // Test 8: status uses the running daemon as relay authority over local IPC
  {
    ...
    const listen =
      process.platform === "win32"
        ? `\\\\.\\pipe\\frogg-status-${process.pid}-${Date.now()}`
        : join(froggHome, "status.sock");
    const configPath = join(froggHome, "config.json");
    ...
    // A supervised daemon owns and heartbeats frogg.pid. Launch the worker
    ...
    const worker = spawn(process.execPath, workerArgs, {
      ...
      env: {
        ...
        FROGG_LISTEN: listen,
        FROGG_LOCAL_SPEECH_AUTO_DOWNLOAD: "0",
    ...
    try {
      ...
      await writeFile(configPath, `${JSON.stringify(reloadConfig, null, 2)}\n`, "utf-8");
      const aliasReload = await runLocalFrogg(["reload", "--host", listen, "--json"], {
        FROGG_HOME: froggHome,
      });
      ...
      const foreignHome = await mkdtemp(join(tmpdir(), "frogg-test-foreign-home-"));
      try {
        await writeFile(
        ...
        const foreignPairing = await retryWhileWorkerRuns(
          worker,
          () =>
            runLocalFrogg(["daemon", "pair", "--home", foreignHome, "--json"], {
              FROGG_HOME: foreignHome,
            }),
          (result) => result.stderr.includes("different Frogg home"),
        ...
        assert(
          foreignPairing.stderr.includes("different Frogg home"),
