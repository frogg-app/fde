 ...
 * 2. Create agent with `fde run "say hello" --provider claude`
 * 3. List agents with `fde ls`
 * 4. Wait for agent with `fde wait <id>`
 * 5. Inspect agent with `fde inspect <id>`
 * 6. Stop agent with `fde stop <id>` and verify it remains inspectable
 * 7. Delete agent with `fde delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`FDE_HOME: ${ctx.fdeHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.fde(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.fde(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.fde(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.fde(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.fde(["stop", agentId]);
  ...
  const inspectResult = await ctx.fde(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.fde(["delete", agentId]);
