 ...
 * 2. Create agent with `frogg run "say hello" --provider claude`
 * 3. List agents with `frogg ls`
 * 4. Wait for agent with `frogg wait <id>`
 * 5. Inspect agent with `frogg inspect <id>`
 * 6. Stop agent with `frogg stop <id>` and verify it remains inspectable
 * 7. Delete agent with `frogg delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`FROGG_HOME: ${ctx.froggHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.frogg(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.frogg(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.frogg(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.frogg(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.frogg(["stop", agentId]);
  ...
  const inspectResult = await ctx.frogg(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.frogg(["delete", agentId]);
