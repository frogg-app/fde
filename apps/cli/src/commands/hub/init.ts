...
export async function runHubGuidedSetup(
  ...
  if (deploy) {
    ...
    reportMessage(
      environment,
      `Skipped deployment. Run: ${brand.cliName} hub deploy -p ${project.slug}`,
    );
  }
...
export async function continueHubGuidedSetup(
  ...
  if (current.kind === "connected") {
    ...
    await requiredConfirm(
      environment,
      `Connect this daemon to ${brand.name} Hub?
...
Enable Hub automations later:
  ${brand.cliName} hub permissions grant hub.execute`,
      );
    }
  } else {
    reportMessage(
  ...
  if (connection.kind === "connected") {
    if (permissions.includes("hub.execute") && !status.permissions.includes("hub.execute")) {
      throw new HubCommandError(
        ...
        `This daemon is connected to Hub but cannot run Hub automations. Run \`${brand.cliName} hub permissions grant hub.execute\`, then run Hub init again.`,
      );
    }
      ...
      while (true) {
        ...
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            ...
            `The daemon did not connect within 60 seconds. Check \`${brand.cliName} hub status\`, then run Hub init again.`,
          );
        }
...
async function chooseProject(
  ...
  if (resolution.kind === "none") {
    throw new HubInitCancelledError(
      `No Hub projects exist yet. Create one at ${new URL("/projects/new", origin).toString()}, then run ${brand.cliName} hub init again.`,
    );
  }
...
Connect GitHub, Slack, or Discord in Hub → Apps, then run \`${brand.cliName} hub init\` again.`,
    );
  }
  return connections;
}
...
function reportStarterTriggerConnections(
  ...
  const details = `${connections.map(({ label }) => label).join("\n")}\n\nOnly configured connections are shown. To add another, open Hub → Apps, then run \`${brand.cliName} hub init\` again.`;
...
async function chooseStarterTriggerConnection(
  ...
  if (connection === undefined) {
    throw new HubCommandError(
      ...
      `The selected Hub app connection is no longer available. Run ${brand.cliName} hub init again.`,
    );
  }
  return connection;
}
...
async function chooseStarterAgentRuntime(
  ...
  if (runtime === undefined) {
    throw new HubCommandError(
      ...
      `The selected starter agent runtime is no longer available. Run ${brand.cliName} hub init again.`,
    );
  }
  return runtime;
}
      ...
      while (true) {
        ...
        if (state.kind === "unavailable") {
          throw new HubCommandError(
            ...
            `No usable agent runtime is available from this daemon. Configure an enabled provider with a selectable model, then run ${brand.cliName} hub init again.`,
          );
        }
        if (Date.now() >= deadline) {
          throw new HubCommandError(
            "HUB_AGENT_RUNTIME_TIMEOUT",
            `Agent runtime discovery did not finish within 60 seconds. Check the daemon's provider configuration, then run ${brand.cliName} hub init again.`,
          );
        }
...
function requireInteractiveTerminal(environment: HubGuidedSetupEnvironment): void {
  if (!(environment.isInteractive?.() ?? (process.stdin.isTTY && process.stdout.isTTY))) {
    throw new HubCommandError(
      "HUB_INIT_INTERACTIVE_REQUIRED",
      `${brand.cliName} hub init requires a TTY.`,
    );
  }
}
