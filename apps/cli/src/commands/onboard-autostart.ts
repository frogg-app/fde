...
/**
 * "Start the Frogg daemon automatically when you log in?" — the same code as
 * `frogg daemon install-service`. Interactive runs are asked (default yes);
 * non-interactive runs honour FROGG_AUTOSTART=0|1 and otherwise change nothing.
 */
export function resolveNonInteractiveAutostart(env: NodeJS.ProcessEnv): boolean | null {
  const raw = env.FROGG_AUTOSTART?.trim().toLowerCase();
