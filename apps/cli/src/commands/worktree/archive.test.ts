...
function createFakeDaemonClient(
  overrides: Partial<
    Pick<DaemonClient, "getFroggWorktreeList" | "archiveFroggWorktree" | "close">
  > = {},
): DaemonClient {
  return {
    getFroggWorktreeList: async () => ({
      worktrees: [],
      error: null,
      requestId: "req-list",
    }),
    archiveFroggWorktree: async () => ({
      success: true,
      removedAgents: [],
      error: null,
...
describe("runArchiveCommand", () => {
  it("sends scope worktree when archiving by worktree path", async () => {
    const worktreePath = "/tmp/frogg-home/worktrees/repo/feature";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveFroggWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
      ...
      archiveFroggWorktree: async (input) => {
  ...
  it("archives by matching branch name when no directory name matches", async () => {
    const worktreePath = "/tmp/frogg-home/worktrees/repo/feature-branch";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveFroggWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
