...
const baseUrl = process.env.FDE_PROFILE_APP_URL ?? "http://localhost:19010";
const digits = parseDigits(process.env.FDE_PROFILE_WORKSPACE_DIGITS ?? "1234567");
const lruSize = Number(process.env.FDE_PROFILE_WORKSPACE_LRU_SIZE ?? 3);
const retainedRepeatCount = Number(process.env.FDE_PROFILE_RETAINED_REPEATS ?? 1);
const warmQuietMs = Number(process.env.FDE_PROFILE_WARM_QUIET_MS ?? 300);
const finalWaitMs = Number(process.env.FDE_PROFILE_FINAL_WAIT_MS ?? 1_000);
const headless = process.env.FDE_PROFILE_HEADLESS !== "0";
const dumpCommits = process.env.FDE_PROFILE_DUMP_COMMITS === "1";
const cpuProfilePath = process.env.FDE_PROFILE_CPU_PATH;
const tracePath = process.env.FDE_PROFILE_TRACE_PATH;
const traceInvalidations = process.env.FDE_PROFILE_TRACE_INVALIDATIONS === "1";
const traceFocus = process.env.FDE_PROFILE_TRACE_FOCUS === "1";

function parseDigits(value) {
  const parsed = [...value].filter((digit) => /^[1-9]$/.test(digit));
  if (parsed.length === 0 || new Set(parsed).size !== parsed.length) {
    throw new Error("FDE_PROFILE_WORKSPACE_DIGITS must contain unique digits from 1 through 9");
  }
  return parsed;
}
  ...
  await page.addInitScript(
    ({ profileFocus }) => {
      localStorage.setItem(
        "@fde:keyboard-shortcut-overrides",
      ...
      if (profileFocus) {
        ...
        HTMLElement.prototype.focus = function (...args) {
          ...
          globalThis.__FDE_FOCUS_PROFILE__ ??= [];
          globalThis.__FDE_FOCUS_PROFILE__.push({
            time: start,
...
async function beginBrowserMeasurements(page, scenarioName) {
  await page.evaluate((name) => {
    ...
    globalThis.__FDE_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__?.();
    globalThis.__FDE_FOCUS_PROFILE__ = [];
    ...
    markTrace(`fde:${name}:capture-start`);
    ...
    const recordActivation = (records) => {
      ...
      markTrace(`fde:${name}:activation:${activationSequence}:${activeEntry}`);
      activationSequence += 1;
      measurements.activations.push({ activeEntry, time: performance.now() });
    };
    const recordKeydown = (digit) => {
      markTrace(`fde:${name}:keydown:${keydownSequence}:Cmd+${digit}`);
      keydownSequence += 1;
    ...
    globalThis.__FDE_WORKSPACE_SWITCH_BENCHMARK__ = measurements;
    globalThis.__FDE_RECORD_WORKSPACE_SWITCH_KEYDOWN__ = recordKeydown;
    globalThis.__FDE_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__ = () => {
      observer.disconnect();
    };
  }, scenarioName);
...
async function runScenario(page, name, scenarioDigits, targets) {
  ...
  for (const digit of scenarioDigits) {
    await page.evaluate((nextDigit) => {
      globalThis.__FDE_RECORD_WORKSPACE_SWITCH_KEYDOWN__?.(nextDigit);
    }, digit);
    await page.keyboard.press(digit);
  }
  await page.keyboard.up("Meta");
  ...
  await page.evaluate((scenarioName) => {
    const label = `fde:${scenarioName}:capture-end`;
    performance.mark(label);
    console.timeStamp(label);
  }, name);

  const result = await page.evaluate(() => ({
    measurements: globalThis.__FDE_WORKSPACE_SWITCH_BENCHMARK__,
    samples: globalThis.__FDE_RENDER_PROFILE__ ?? [],
    reasons: globalThis.__FDE_RENDER_PROFILE_REASONS__ ?? {},
    focusCalls: globalThis.__FDE_FOCUS_PROFILE__ ?? [],
  }));
  await page.evaluate(() => globalThis.__FDE_WORKSPACE_SWITCH_BENCHMARK_CLEANUP__?.());
  return {
