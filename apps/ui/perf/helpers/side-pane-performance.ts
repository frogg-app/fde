...
const APP_SETTINGS_KEY = "@frogg:app-settings";
...
declare global {
  interface Window {
    __FROGG_SIDE_PANE_MEASUREMENT__?: BrowserMeasurementState;
  }
}
...
export function resolveProfileTarget(): ProfileTarget {
  ...
  const serverId = (
    ...
    readFileSync(resolve(repoRoot, ".dev/frogg-home/server-id"), "utf8")
  ).trim();
  const requestedName = process.env.FROGG_PROFILE_WORKSPACE_NAME?.trim() || "Frogg";
  const requestedWorkspaceId = process.env.FROGG_PROFILE_WORKSPACE_ID?.trim();
  ...
  if (!workspace) {
    throw new Error(`No Frogg workspace named ${requestedName} points at ${repoRoot}`);
  }
...
async function beginMeasurement(page: Page, selectors: string[]): Promise<void> {
  ...
  await page.evaluate((trackedSelectors) => {
    ...
    window.__FROGG_SIDE_PANE_MEASUREMENT__ = state;
  }, selectors);
}
...
async function finishMeasurement(page: Page, name: string): Promise<InteractionMeasurement> {
  ...
  const raw = await page.evaluate((measurementName) => {
    const state = window.__FROGG_SIDE_PANE_MEASUREMENT__;
    if (!state) throw new Error("Side-pane performance measurement was not started");
    state.observer?.disconnect();
    const samples = globalThis.__FROGG_RENDER_PROFILE__ ?? [];
    return {
