...
const APP_SETTINGS_KEY = "@fde:app-settings";
...
declare global {
  interface Window {
    __FDE_SIDE_PANE_MEASUREMENT__?: BrowserMeasurementState;
  }
}
...
export function resolveProfileTarget(): ProfileTarget {
  ...
  const serverId = (
    ...
    readFileSync(resolve(repoRoot, ".dev/fde-home/server-id"), "utf8")
  ).trim();
  const requestedName = process.env.FDE_PROFILE_WORKSPACE_NAME?.trim() || "Fde";
  const requestedWorkspaceId = process.env.FDE_PROFILE_WORKSPACE_ID?.trim();
  ...
  if (!workspace) {
    throw new Error(`No Fde workspace named ${requestedName} points at ${repoRoot}`);
  }
...
async function beginMeasurement(page: Page, selectors: string[]): Promise<void> {
  ...
  await page.evaluate((trackedSelectors) => {
    ...
    window.__FDE_SIDE_PANE_MEASUREMENT__ = state;
  }, selectors);
}
...
async function finishMeasurement(page: Page, name: string): Promise<InteractionMeasurement> {
  ...
  const raw = await page.evaluate((measurementName) => {
    const state = window.__FDE_SIDE_PANE_MEASUREMENT__;
    if (!state) throw new Error("Side-pane performance measurement was not started");
    state.observer?.disconnect();
    const samples = globalThis.__FDE_RENDER_PROFILE__ ?? [];
    return {
