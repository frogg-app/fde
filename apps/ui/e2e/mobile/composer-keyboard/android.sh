...
STATE_DIR="${FROGG_COMPOSER_KEYBOARD_STATE_DIR:-${REPO_ROOT}/.dev/agent-device-composer-keyboard}"
ARTIFACTS_DIR="${REPO_ROOT}/.dev/agent-device-artifacts/composer-keyboard-android"
SESSION="${FROGG_COMPOSER_KEYBOARD_SESSION:-composer-keyboard-android}"
APP_ID="${FROGG_COMPOSER_KEYBOARD_APP_ID:-sh.frogg.debug}"
DEVICE="${FROGG_COMPOSER_KEYBOARD_DEVICE:-frogg-api35}"
...
ASSERT="${REPO_ROOT}/apps/ui/e2e/mobile/composer-keyboard/assert-composer-keyboard.mjs"
STALL_HERMES="${REPO_ROOT}/apps/ui/e2e/mobile/composer-keyboard/stall-hermes.mjs"
METRO_PORT="${FROGG_MOBILE_E2E_METRO_PORT:-8082}"
DAEMON_HOST="${FROGG_COMPOSER_KEYBOARD_DAEMON_HOST:-127.0.0.1:6770}"
DAEMON_HOME="${FROGG_COMPOSER_KEYBOARD_DAEMON_HOME:-${REPO_ROOT}/.dev/composer-e2e-home}"
SERVER_ID="${FROGG_COMPOSER_KEYBOARD_SERVER_ID:-}"
...
if [[ -z "${SERVER_ID}" ]]; then
  if [[ ! -f "${DAEMON_HOME}/server-id" ]]; then
    echo "Missing ${DAEMON_HOME}/server-id; set FROGG_COMPOSER_KEYBOARD_SERVER_ID" >&2
    exit 1
  fi
...
capture_ui_xml() {
  local output_path="$1"
  local device_path="/sdcard/frogg-composer-keyboard-window.xml"
...
workspaces_json="$(env -u FROGG_CALLER_AGENT_ID -u FROGG_AGENT_ID -u FROGG_WORKSPACE_ID \
...
run_json="$(env -u FROGG_CALLER_AGENT_ID -u FROGG_AGENT_ID -u FROGG_WORKSPACE_ID \
...
adb shell am start \
  -a android.intent.action.VIEW \
  -d "frogg://h/${SERVER_ID}/agent/${agent_id}" \
  "${APP_ID}" >/dev/null
sleep 5
