...
STATE_DIR="${FDE_COMPOSER_KEYBOARD_STATE_DIR:-${REPO_ROOT}/.dev/agent-device-composer-keyboard}"
ARTIFACTS_DIR="${REPO_ROOT}/.dev/agent-device-artifacts/composer-keyboard-android"
SESSION="${FDE_COMPOSER_KEYBOARD_SESSION:-composer-keyboard-android}"
APP_ID="${FDE_COMPOSER_KEYBOARD_APP_ID:-sh.fde.debug}"
DEVICE="${FDE_COMPOSER_KEYBOARD_DEVICE:-fde-api35}"
...
METRO_PORT="${FDE_MOBILE_E2E_METRO_PORT:-8082}"
DAEMON_HOST="${FDE_COMPOSER_KEYBOARD_DAEMON_HOST:-127.0.0.1:6770}"
DAEMON_HOME="${FDE_COMPOSER_KEYBOARD_DAEMON_HOME:-${REPO_ROOT}/.dev/composer-e2e-home}"
SERVER_ID="${FDE_COMPOSER_KEYBOARD_SERVER_ID:-}"
...
if [[ -z "${SERVER_ID}" ]]; then
  if [[ ! -f "${DAEMON_HOME}/server-id" ]]; then
    echo "Missing ${DAEMON_HOME}/server-id; set FDE_COMPOSER_KEYBOARD_SERVER_ID" >&2
    exit 1
  fi
...
capture_ui_xml() {
  local output_path="$1"
  local device_path="/sdcard/fde-composer-keyboard-window.xml"
...
workspaces_json="$(env -u FDE_CALLER_AGENT_ID -u FDE_AGENT_ID -u FDE_WORKSPACE_ID \
...
run_json="$(env -u FDE_CALLER_AGENT_ID -u FDE_AGENT_ID -u FDE_WORKSPACE_ID \
...
adb shell am start \
  -a android.intent.action.VIEW \
  -d "fde://h/${SERVER_ID}/agent/${agent_id}" \
  "${APP_ID}" >/dev/null
sleep 5
