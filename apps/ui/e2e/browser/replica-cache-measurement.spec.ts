...
const RUN_MEASUREMENT = process.env.FROGG_REPLICA_CACHE_MEASUREMENT === "1";
const TARGET_URL = process.env.FROGG_REPLICA_CACHE_MEASUREMENT_URL;
const REPORT_PATH = process.env.FROGG_REPLICA_CACHE_MEASUREMENT_REPORT;
const OBSERVATION_MS = Number(process.env.FROGG_REPLICA_CACHE_MEASUREMENT_MS ?? 60_000);
const HYDRATED_SELECTOR =
  process.env.FROGG_REPLICA_CACHE_HYDRATED_SELECTOR ??
...
measurementDescribe("Replica cache live measurement", () => {
  test("observes persistence and cached restore without mutating the daemon", async ({
    ...
    expect(TARGET_URL, "FROGG_REPLICA_CACHE_MEASUREMENT_URL must be a full app URL").toBeTruthy();
    expect(REPORT_PATH, "FROGG_REPLICA_CACHE_MEASUREMENT_REPORT must be set").toBeTruthy();
