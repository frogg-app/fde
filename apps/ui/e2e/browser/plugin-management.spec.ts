...
export default function contribute(plugin) {
  function Surface() {
    ...
    const cleanups = Number(globalThis.sessionStorage?.getItem("fde-plugin-cleanups") || "0");
  ...
  return () => {
    ...
    if (storage) {
      const cleanups = Number(storage.getItem("fde-plugin-cleanups") || "0");
      storage.setItem("fde-plugin-cleanups", String(cleanups + 1));
    }
  };
}`;
}
...
test("installs, reloads, recovers, disables, and removes a trusted local plugin", async ({
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "fde-plugin-e2e-"));
  const client = await connectNewWorkspaceDaemonClient({ ownProjects: false });
  const previous = await client.getDaemonConfig();
  await writeFile(path.join(directory, "fde-plugin.json"), JSON.stringify({ id: "e2e-plugin" }));
