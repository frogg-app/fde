...
Start the `sh.fde.debug` dev client against the intended Metro server first;
the flow preserves that running connection.

```bash
maestro test apps/ui/maestro/ios-sidebar-close-regression.yaml --udid <simulator-udid>
```
...
```bash
bash apps/ui/maestro/test-sidebar-drag-cancellation-ios.sh
```
...
working with `${FDE_MAESTRO_*}` placeholders.
...
The scripts assume a development build with package id `sh.fde.debug`, an
...
```bash
FDE_MAESTRO_APP_ID=sh.fde.debug
FDE_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:9999
FDE_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:9999/ws
FDE_MAESTRO_PROJECT_PATH=/path/to/git/repo
```
