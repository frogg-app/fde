...
const RESIDENT_HOST_ID = "fde-browser-resident-webviews";
...
const profileHost: BrowserWebviewProfileHost = {
  profilePartition: "persist:fde-browser",
...
describe("resident browser webviews", () => {
  ...
  it("creates a resident webview for an agent-created unfocused tab", () => {
    ...
    expect(webview?.isConnected).toBe(true);
    expect(webview?.getAttribute("data-fde-browser-id")).toBe("browser-agent");
    expect(webview?.getAttribute("partition")).toBe("persist:fde-browser");
  ...
  it("shares one profile and registers attached guests with explicit identity", () => {
    ...
    expect(firstWebview.getAttribute("partition")).toBe("persist:fde-browser");
    expect(secondWebview.getAttribute("partition")).toBe("persist:fde-browser");
