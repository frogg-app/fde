  ...
  it("persists the selected route for the host", () => {
    const scripts = [
      script({
        ...
        lifecycle: "running",
        port: 57483,
        proxyUrl: "http://dev--proj--repo.localhost:9999",
        terminalId: "terminal-script-1",
      }),
    ];
    ...
    expect(row.textContent).toContain("dev--proj--repo.localhost:9999");
  ...
  it("defaults to a configured reverse proxy URL", () => {
    current = renderScripts([
      script({
        ...
        lifecycle: "running",
        port: 57483,
        localProxyUrl: "http://dev--proj--repo.localhost:9999",
  ...
  it("adds localized tooltips to every icon action", () => {
    current = renderScripts([
      script({
        ...
        lifecycle: "running",
        port: 3000,
        proxyUrl: "http://dev--project.localhost:9999",
        terminalId: "terminal-script-1",
      }),
    ]);
