...
import { FroggConfigRawSchema } from "@frogg/protocol/frogg-config-schema";
...
describe("applyDraftToConfig", () => {
  it("preserves the original string kind when editing an existing setup field", () => {
    const base: FroggConfigRaw = { worktree: { setup: "npm install" } };
  ...
  it("preserves unknown top-level, worktree, and script entry fields on round-trip", () => {
    const base = FroggConfigRawSchema.parse({
      worktree: {
        setup: "npm install",
