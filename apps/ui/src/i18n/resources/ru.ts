    ...
    host: {
      ...
      sshDeploy: {
        ...
        fields: {
          ...
          listenHint:
            "Приложение подключается через SSH-туннель, поэтому loopback — правильный выбор. Используйте 0.0.0.0 только чтобы открыть демон в сеть.",
          version: "Версия",
          versionHint:
      ...
      daemon: {
        ...
        selfUpdate: {
          ...
          confirmMessage: "Демон перезапустится на новую версию; работающие агенты ненадолго прервутся. Если новая версия не запустится, предыдущая будет восстановлена автоматически.",
          confirm: "Обновить",
