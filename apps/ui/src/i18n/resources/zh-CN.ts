...
export const zhCN: TranslationResources = {
  ...
  spokenAlerts: {
    ...
    reply: {
      ...
      allow: "允许",
      deny: "拒绝",
      sendAsMessage: "作为消息发送",
      willAllow: "将允许此请求。",
      willDeny: "将拒绝此请求。",
      ambiguous: "无法判断是“是”还是“否”。请选择操作。",
      permissionPrompt: "代理正在等待权限决定。说“是”或“否”，或口述一条回复。",
      keepListening: "继续聆听下一条提醒",
    },
  },
  rewind: {
    tooltip: "回退到此消息",
  ...
  settings: {
    ...
    voiceAlerts: {
      ...
      replyConfirm: {
        label: "确认语音回复",
        description: "发送前显示转写内容两秒，以便编辑或取消。",
      },
    },
    general: {
      title: "通用",
    ...
    host: {
      ...
      sshDeploy: {
        ...
        probe: {
          ...
          docker: {
            label: "Docker",
            available: "可用",
            container: "可用，存在 frogg-daemon 容器",
            missing: "不可用",
          },
          curlMissing: "主机上没有 curl；原生安装需要它来下载发布包。",
          noService: "没有用户服务管理器，守护进程在重启后不会自动启动。",
          installDir: "原生安装位置：{{home}}/.local/share/frogg",
        ...
        uninstall: {
          title: "卸载守护进程？",
          message:
            "停止服务并从 {{host}} 移除 {{brandName}} 安装。守护进程状态 (~/.frogg) 会保留。",
        },
        log: {
          running: "正在 {{host}} 上运行...",
          done: "已完成。",
      ...
      relayEndpoint: {
        ...
        saveFailed: "无法保存中继端点。",
        overridden:
          "由守护进程启动覆盖项（FROGG_RELAY_ENDPOINT 或 FROGG_RELAY_USE_TLS）设置。请在启动守护进程的位置修改。",
      },
      skills: {
      ...
      daemon: {
        ...
        selfUpdate: {
          title: "守护进程更新",
          info: "从 GitHub 发布版更新通过 install.sh（或桌面应用的 SSH 部署）安装的守护进程。守护进程会把新版本下载到当前版本旁边并重启到新版本；如果新版本无法启动，会自动回退到上一个版本。",
          versionLabel: "守护进程 {{version}}",
          versionUnknown: "守护进程版本未知",
          hint: "检查发布渠道中是否有更新的守护进程",
          check: "检查守护进程更新",
          checking: "正在检查...",
          upToDate: "守护进程已是最新版本",
          ...
          outcome: {
            applied: "已从 {{from}} 更新到 {{to}}",
            rolled_back: "更新到 {{to}} 失败，已回退到 {{from}}",
            failed: "从 {{from}} 更新到 {{to}} 失败",
          },
          autoUpdate: {
            title: "自动更新",
    ...
    project: {
      ...
      worktree: {
        ...
        uncommittedTitle: "提交 frogg.json 更改",
      ...
      scripts: {
        ...
        editScript: "编辑 {{name}}",
        runAsService: "作为服务运行",
        serviceHint: "{{brandName}} 会监管该进程，并通过 $FROGG_PORT 分配端口",
        actions: {
          add: "添加 script",
          edit: "编辑",
          remove: "移除",
      ...
      writeFailures: {
        ...
        staleDescription: "保存前请重新加载最新的 frogg.json。",
        failedTitle: "无法保存 frogg.json",
