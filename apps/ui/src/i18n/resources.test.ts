...
function findInterpolationMismatches(resource: unknown): string[] {
  ...
  return Object.entries(englishStrings).flatMap(([key, value]) => {
    const expected = [...value.matchAll(interpolationPattern)]
      .map((match) => match[0])
      .filter(
        (placeholder) =>
          !["{{brandName}}", "{{brandFullName}}", "{{cliName}}"].includes(placeholder),
      )
      .sort();
...
describe("translation resources", () => {
  ...
  it("includes Settings expansion keys for the Batch 3B migration", () => {
    ...
    expect(en.settings.host.orchestration.enableTools.title).toBe("Enable {{brandName}} tools");
  ...
  it("includes onboarding and direct connection keys for the Batch 4E migration", () => {
    expect(en.onboarding.title).toBe("Welcome to {{brandName}}");
  ...
  it("includes remaining small utility chrome keys for the Batch 4O migration", () => {
    ...
    expect(en.desktop.quitting.title).toBe("Quitting {{brandName}}...");
  ...
  it("includes desktop permission utility keys for the Batch 4R migration", () => {
    ...
    expect(en.desktop.permissions.testNotification.title).toBe("{{brandName}} notification test");
