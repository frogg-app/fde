...
// Regression: "streaming pino log, resized the Fde terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__fdeTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (Fde terminal)", () => {
