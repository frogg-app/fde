...
// Regression: "streaming pino log, resized the Frogg terminal, old logs stayed
...
function mount(width: number, height: number): Mounted {
  ...
  const terminal = window.__froggTerminal as InspectableTerminal | undefined;
  if (!terminal) {
...
describe("terminal resize reflow repro (Frogg terminal)", () => {
