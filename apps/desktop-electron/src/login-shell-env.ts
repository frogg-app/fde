...
const TIMEOUT_ENV_KEY = "FDE_SHELL_ENV_TIMEOUT_MS";
...
function resolveShellEnv({ deps, timeoutMs }: ResolveShellEnvInput): ResolvedShellEnv {
  if (deps.platform === "win32") {
    throw new ShellEnvError("login shell env is not resolved on Windows", {
      reason: "win32",
    });
  }
  ...
  delete shellEnv.FDE_NODE_ENV;
  delete shellEnv.FDE_DESKTOP_MANAGED;
  delete shellEnv.FDE_SUPERVISED;
