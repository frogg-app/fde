...
class FakeTab implements TabContents {
  ...
  public readonly debugCommands: Array<{
    command: string;
    ...
    root: {
      ...
      children: nodes.map((node) => ({
        ...
        ...(node.ref
          ? {
              ref: node.ref,
              fingerprint: refs.find((ref) => ref.ref === node.ref)?.fingerprint,
            }
          : {}),
...
describe("executeAutomationCommand", () => {
  ...
  test.each([
    ...
    },
    {
      ...
      command: {
        command: "select",
        args: { browserId: BROWSER_A, ref: "@e3", value: "us" },
      },
      result: {
        command: "select",
        browserId: BROWSER_A,
  ...
  test("keypress reports unsupported when focusing an editable ref requires unavailable CDP", async () => {
    ...
    requireSnapshotRefs(await browser.snapshot());
    Object.defineProperty(browser.tab, "sendDebugCommand", {
      value: undefined,
    });
  ...
  test.each([
    {
      command: {
        command: "resize",
        args: { browserId: BROWSER_A, width: 1024, height: 768 },
      },
  ...
  test("evaluate returns object JSON from the page context", async () => {
    ...
    const result = await browser.execute({
      ...
      args: {
        browserId: BROWSER_A,
        function: "() => ({ title: document.title, ready: true })",
      },
    });
  ...
  test("upload resolves workspace files before setting them on the file input", async () => {
    ...
    expect(browser.tab.debugCommands).toEqual([
      ...
      { command: "DOM.describeNode", params: { objectId: "object-1" } },
      {
        ...
        params: {
          backendNodeId: 2,
          files: [resolvePath(workspaceRoot, "uploads/a.txt")],
        },
      },
    ]);
  });
