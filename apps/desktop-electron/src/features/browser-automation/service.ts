...
async function executeFill(
  ...
  return withDialogCapture(target.contents, async () => {
    ...
    return {
      requestId,
      ok: true,
      result: { command: "fill", browserId: target.browserId, ref },
    };
  });
}
  ...
  return withDialogCapture(target.contents, async () => {
    if (action === "back") {
      ...
      return {
        requestId,
        ok: true,
        result: { command: "back", browserId: target.browserId },
      };
    }
    if (action === "forward") {
      ...
      return {
        requestId,
        ok: true,
        result: { command: "forward", browserId: target.browserId },
      };
    }
    snapshotEngine.clearBrowser(browserId);
    ...
    return {
      requestId,
      ok: true,
      result: { command: "reload", browserId: target.browserId },
    };
  });
}
...
function readEvaluateScriptResult(value: unknown): EvaluateScriptResult {
  if (!value || typeof value !== "object") {
    return {
      status: "error",
      message: "Browser evaluate returned an invalid result.",
    };
  }
  const record = value as Record<string, unknown>;
...
function capEvaluateResultJson(resultJson: string): {
  resultJson: string;
  truncated: boolean;
} {
