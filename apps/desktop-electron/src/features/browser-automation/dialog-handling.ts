import type { BrowserAutomationDialogEvent } from "@fde/protocol/browser-automation/rpc-schemas";
...
export function promptShimInstallScript(): string {
  return String.raw`(() => {
    const stateKey = "__FDE_BROWSER_AUTOMATION_DIALOG_STATE__";
...
export function promptShimDrainScript(): string {
  return String.raw`(() => {
    const state = window.__FDE_BROWSER_AUTOMATION_DIALOG_STATE__;
