...
This page covers working on the FDE monorepo itself. Commands run from the repository root
...
```
protocol → client → highlight, plugin, relay → server → cli
```
...
| Script                    | Builds                                                         |
| ------------------------- | -------------------------------------------------------------- |
| `npm run build:protocol`  | `@fde/protocol`                                                |
| `npm run build:client`    | `@fde/protocol`, then `@fde/client`                            |
| `npm run build:server-deps` | client chain, then highlight, plugin and relay in parallel  |
| `npm run build:server`    | server deps, then `@fde/server` and `@fde/cli`                 |
| `npm run build:app-deps`  | highlight, client, plugin and `@fde/expo-two-way-audio`        |
...
- `dev:app` sets `EXPO_PUBLIC_LOCAL_DAEMON` to the dev daemon endpoint and passes the current
  git branch as `EXPO_PUBLIC_FDE_DEV_BUILD_LABEL`. `EXPO_PUBLIC_*` values are inlined at bundle
...
- `npm run dev:desktop` runs the Electron shell against a web dev server on port
  `18000 + <offset derived from the checkout path>` (override with `FDE_ELECTRON_UI_PORT`).
...
`FDE_BRAND_DIR` uses its `daemonPort` and `daemonPort + 1` instead. Port 9999 is the default
port of a separately installed daemon; keep it free of dev work.

## Dev state

`FDE_HOME` holds agents, projects, worktrees, `config.json` and `daemon.log`. The dev scripts
default it to `.dev/fde-home` inside the current checkout, so dev work never touches `~/.fde`.

```bash
FDE_HOME=~/.fde-blue npm run dev:server                 # explicit home
FDE_DEV_SEED_HOME=~/.fde npm run dev:server             # copy JSON metadata from another home
FDE_DEV_SEED_HOME=~/.fde FDE_DEV_RESET_HOME=1 npm run dev:server   # clear, then reseed
```

Seeding copies `agents/**/*.json`, `projects/**/*.json` and `config.json`. It skips a home that
already has data unless `FDE_DEV_RESET_HOME=1` is set.

:::caution
In `scripts/dev/dev-home.sh`, `FDE_DEV_RESET_HOME` only takes effect together with
`FDE_DEV_SEED_HOME`. On its own it does nothing.
:::
...
the checkout's `.dev/fde-home` and the dev daemon endpoint. An installed `fde` binary is a
different build; use `npm run cli` to test CLI changes.

## Several checkouts at once

Each checkout or worktree gets its own `.dev/fde-home` automatically. Give each one its own
ports:

```bash
FDE_LISTEN=0.0.0.0:6868 npm run dev:server
FDE_LISTEN=0.0.0.0:6868 EXPO_PORT=8181 npm run dev:app
```

`dev:app` derives the daemon endpoint from `FDE_LISTEN`. Set `FDE_DEV_DAEMON_ENDPOINT` to point
it somewhere else. `npm run worktree:status` lists what every worktree has committed and left
uncommitted.

## Working on FDE inside FDE

The repo's own `fde.json` configures worktrees created by an FDE daemon. See
...
`scripts/dev/seed-worktree-dev-state.mjs` (copies the source checkout's `.dev/fde-home` and
`packages/server/.env`), `npm run build:server`, and the `@fde/expo-two-way-audio` build.

| Script          | Type    | What it runs                                                              |
| --------------- | ------- | ------------------------------------------------------------------------- |
| `daemon`        | service | `scripts/dev/dev-daemon.sh` on `FDE_PORT`, skipping the deps build        |
| `app`           | service | `scripts/dev/dev-app.sh` with Metro on `FDE_PORT`, pointed at the `daemon` service |
| `desktop`       | service | `./packages/desktop/scripts/dev.sh`                                       |
| `ios-simulator` | service | `scripts/mobile/fde-ios-simulator-service.sh` (macOS only)                |
...
- On a fresh checkout `npm run typecheck` fails: `@fde/app` cannot resolve `@fde/client` and
  `@fde/cli` cannot resolve `@fde/server`. Run `npm run build:server` and
...
- `pretypecheck` runs `brand:prepare` and builds `@fde/expo-two-way-audio`.
...
Output goes to `apps/desktop-electron/release/`. macOS artifacts must be built on macOS
...
npm run android --workspace=@fde/app   # dev client via expo run:android
```

`--serial` limits Gradle to one worker for machines with about 8 GB of RAM. Without the
`FDE_ANDROID_KEYSTORE*` variables the APK is debug-signed; see
[Release process](/docs/contributing/release-process/#android-signing).

**iOS.** macOS only. Needs Xcode with an iOS Simulator runtime. `npm run ios --workspace=@fde/app`
builds and launches the dev client with `expo run:ios`. CI builds an unsigned simulator app in
`branding.yml`. FDE does not publish iOS builds.
